...
const output = 'work/desktop-parity/backing-fold';
...
try {
  for (const width of (process.env.BACKING_TEST_WIDTHS || '412,1440').split(',').map(Number)) {
    ...
    try {
      ...
      for(let back=0;back<2&&await page.getByRole('button',{name:'뒤로',exact:true}).isVisible();back++) await page.getByRole('button',{name:'뒤로',exact:true}).click();
      await page.locator('.rt-card').first().waitFor();
