...
try{
 for(const [width,height,theme] of [[390,844,'light'],[360,800,'brand'],[1440,1000,'light'],[1440,900,'brand']]){
  ...
  await click(p,'카포 없음 ▾');await p.getByRole('dialog',{name:'카포 설정',exact:true}).getByRole('button',{name:'2',exact:true}).click();
  ...
  await click(p,'카포 2 ▾');await p.getByRole('dialog',{name:'카포 설정',exact:true}).getByRole('button',{name:'3',exact:true}).click();assert(await p.getByRole('radio',{name:'음높이 유지',exact:true}).isChecked());await click(p,'운지 재계산 후 적용');assert.equal(await p.locator('.editorTabWarning').count(),0);
  ...
  await click(p,'카포 2 ▾');await p.getByRole('dialog',{name:'카포 설정',exact:true}).getByRole('button',{name:'3',exact:true}).click();await p.getByRole('radio',{name:'운지 유지',exact:true}).check();await click(p,'변경 적용');d=await save(p);assert.equal(d.measures[0].events[0].notes[0].midi,55);assert.equal(d.measures[0].events[0].notes[0].fret,12);
  await click(p,'편집 ▾');await click(p,'건반 연결');await p.getByText('연결됨 · Test Keyboard a',{exact:true}).waitFor();assert.equal(await p.evaluate(()=>midiCalls),1);
  ...
  await canvas.focus();await click(p,'다음 마디');await canvas.focus();await send(p,[[0x90,76,100],[0x80,76,0]]);const reopened=await save(p);assert.equal(reopened.measures[1].events[0].notes.length,1);assert(reopened.measures[1].events[1].blank);
  await p.screenshot({path:`output/tuning-${width}-${theme}.png`});
  if(width<600){const keypad=await p.locator('.mobileFretPad').boundingBox().catch(()=>null),dock=await p.locator('.editorAudioDock').boundingBox();assert(dock.x>=0&&dock.x+dock.width<=width+1);await p.setViewportSize({width:844,height:390});await click(p,'편집 ▾');const pop=await p.locator('.editorSettingsPopover').boundingBox();assert(pop.x>=0&&pop.y>=0&&pop.y+pop.height<=391);await p.screenshot({path:`output/tuning-landscape-${theme}.png`});}
 ...
 }
 {
  ...
  await click(p,'편집 ▾');await p.getByLabel('자동 TAB 포지션').selectOption('range');await p.getByLabel('선호 최소 프렛').fill('3');await p.getByLabel('선호 최대 프렛').fill('7');await click(p,'건반 연결');await p.getByText('연결됨 · Test Keyboard a',{exact:true}).waitFor();await p.keyboard.press('Escape');await p.locator('[data-score-input]').focus();await send(p,[[0x90,20,100],[0x80,20,0]]);
 ...
 for(const permission of ['granted','denied','rejected','unknown','unsupported']){
  const {page:p,errors}=await setup(390,844,'light',permission);await click(p,'편집 ▾');
  if(permission==='granted'){await p.getByText('연결됨 · Test Keyboard a',{exact:true}).waitFor();assert.equal(await p.evaluate(()=>midiCalls),1);}
  else if(permission==='unsupported'){assert(await p.getByRole('button',{name:'건반 연결'}).isDisabled());assert.equal(await p.evaluate(()=>midiCalls),0);}
  else{assert.equal(await p.evaluate(()=>midiCalls),0);await click(p,'건반 연결');if(permission==='denied'||permission==='rejected'){await p.getByText(/권한 거부:/).waitFor();await click(p,'건반 연결');assert.equal(await p.evaluate(()=>midiCalls),permission==='rejected'?1:0);}else{await p.getByText('연결됨 · Test Keyboard a',{exact:true}).waitFor();assert.equal(await p.evaluate(()=>midiCalls),1);}}
