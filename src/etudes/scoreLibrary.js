...
export function loadLibrary(storage,bases=[]) {
 try{
  ...
  for(const [key,old] of Object.entries(previous)){const base=bases.find(b=>b.templateId===key);try{const legacy=old.version===1&&base?compileScoreDocument(old,base):null;const document=legacy?.score?toScoreDocument(legacy.score):old.version===2?normalizeInstrumentDocument(old):upgradeDocument(old);if(old.version===1)document.keySignature=base?.keySignature??'C';records[document.id]={document,status:'draft',updatedAt:null};}catch{records[`unreadable-${key}`]={document:{id:`unreadable-${key}`,title:`이전 악보 ${key}`},raw:old,status:'unreadable',updatedAt:null};}}
...
export function uniqueLibraryTitle(records,title,id){
 const base=String(title??'').trim().slice(0,200)||'새 악보';
