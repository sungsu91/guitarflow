...
const Measure=memo(function Measure({notes,chord,harmony,index,keySignature,meter,mobile,incomingTie,selection,beamSelection,onSelect,view,tabRhythm,playPosition,playingRow,allowDrag,placement,layoutEdit,breakBefore,onBreak,systemStart,systemEnd,scoreEnd,systemHeadroom,engraving,repeatStart,repeatEnd,marker,command,ending,previousEnding,nextEnding,systemNavigation}) {
 ...
 const spacingKey=`${engraving.inset}/${engraving.tickScale}/${repeatStart}/${repeatEnd}/${marker}/${command}/${ending}/${previousEnding}/${nextEnding}/${systemNavigation}`;
 ...
 useLayoutEffect(()=>{const start=performance.now();try{
  ...
  drawScore(content,{id:`editor-${index}`,title:'편집 마디',bpm:60,keySignature,meter,incomingTie,measures:[notes],repeatMarks:[{repeatStart,repeatEnd,marker,command,ending}],navigationPrevious:{ending:previousEnding},navigationNext:{ending:nextEnding},chordShapes:chord?[chord]:undefined,harmony:[harmony]},{mobile,editor:true,barOffset:index,tabRhythm,view,editorWidth,engraving,systemStart,systemEnd,scoreEnd,systemHeadroom,systemNavigation});const style=document.createElement('style');style.textContent=engravingStyle;root.append(style);setError('');
 ...
 },[notes,chord,harmony,index,keySignature,meter,mobile,incomingTie,tabRhythm,view,editorWidth,systemStart,systemEnd,scoreEnd,systemHeadroom,spacingKey]);
 useLayoutEffect(()=>{
 ...
 },[notes,selection,keySignature,view,editorWidth,systemStart,systemEnd,scoreEnd,systemHeadroom,spacingKey]);
 useLayoutEffect(()=>{
...
export default function EditorScore({score,mobile,cursor,beamSelection,beamRangeMode,onBeamRange,onSelect,onKeyDown,onMove,onMessage,zoom=100,onZoomChange,zoomController,view='both',pageView=false,onLayoutChange,tabRhythm=false,playPosition=null,allowDrag=true}) {
 ...
 const spacing=useMemo(()=>scoreSpacing(score,{placements,view,width:mobile?(view==='both'?600:400):980}),[score,placements,view,mobile]);
