...
const engravingStyle=`:host([data-view=staff]) :is(.vf-fretiva-tab-view,.fretiva-tab-view,[data-mode=tab]),:host([data-view=tab]) :is(.vf-fretiva-staff-view,[data-mode=staff]),:host([data-view=staff]) .vf-fretiva-both-view,:host([data-view=tab]) .vf-fretiva-both-view{display:none}:host{display:block}svg{color:#111;background:white}text{fill:#111}.etudeMeasureNumber{font:700 13px Arial;fill:#111}.etudeEditorHit{cursor:crosshair;pointer-events:all;fill:transparent;stroke:none}.etudeEditorHit:hover{fill:#58968018}.etudeNoteHandle{cursor:grab;touch-action:none}.etudeEditorHit.is-cursor{fill:#2f89672a;stroke:#157251;stroke-width:2;vector-effect:non-scaling-stroke}`;
const Measure=memo(function Measure({notes,chord,harmony,index,keySignature,meter,mobile,incomingTie,selection,onSelect,view}) {
 ...
 useLayoutEffect(()=>{const start=performance.now();try{
  ...
  drawScore(content,{id:`editor-${index}`,title:'편집 마디',bpm:60,keySignature,meter,incomingTie,measures:[notes],chordShapes:chord?[chord]:undefined,harmony:[harmony]},{mobile,editor:true,barOffset:index});const style=document.createElement('style');style.textContent=engravingStyle;root.append(style);setError('');
 ...
 },[notes,chord,harmony,index,keySignature,meter,mobile,incomingTie]);
 useLayoutEffect(()=>{
 ...
 const click=e=>{const hit=e.nativeEvent.composedPath().find(node=>node.dataset?.event!==undefined);if(!hit){onSelect({bar:index,event:0,string:selection?.string??6,mode:'tab',target:'bar'});e.currentTarget.closest('[data-score-input]')?.focus({preventScroll:true});return;}const mode=hit.dataset.mode;let midi;
  ...
  onSelect({bar:index,event:Number(hit.dataset.event),string:Number(hit.dataset.string??1),mode,midi:hit.dataset.midi!==undefined?Number(hit.dataset.midi):midi});e.currentTarget.closest('[data-score-input]')?.focus({preventScroll:true});
 };
 return <section data-bar-index={index} className="etudeEditorMeasure" onClick={click}>{error&&<p role="alert">{error}</p>}<div ref={ref}/></section>;
});
export default function EditorScore({score,mobile,cursor,onSelect,onKeyDown,onMove,onMessage,zoom=100,view='both',pageView=false}) {
 const ref=useRef(null);
 const dragHandlers=useScoreDrag({canvas:ref,score,onSelect,onMove,onMessage});
 useLayoutEffect(()=>{const canvas=ref.current,selected=[...canvas.querySelectorAll('[data-draw-count]')].map(host=>host.shadowRoot?.querySelector('.etudeStaffCursor,.is-cursor')).find(Boolean);if(!selected)return;const a=selected.getBoundingClientRect(),b=canvas.getBoundingClientRect();if(a.top<b.top+12)canvas.scrollTop+=a.top-b.top-12;else if(a.bottom>b.bottom-12)canvas.scrollTop+=a.bottom-b.bottom+12;if(a.left<b.left+12)canvas.scrollLeft+=a.left-b.left-12;else if(a.right>b.right-12)canvas.scrollLeft+=a.right-b.right+12;},[cursor,zoom]);
 ...
 return <div ref={ref} className="etudeEditorCanvas" data-score-input tabIndex={0} role="group" aria-label="악보 키보드 입력" onKeyDown={onKeyDown} {...dragHandlers}>
  <div style={{width:mobile?`max(${zoom}%, ${4.6*zoom}px)`:`${zoom}%`}}>
   {score.measures.map((notes,i)=>(!pageView||i===cursor.bar)&&<Measure key={score.document.measures[i].id} {...{notes,index:i,keySignature:score.keySignature,meter:score.meter,mobile,onSelect,view}} incomingTie={Boolean(i&&score.measures[i-1].at(-1)?.tieTo===notes[0]?.id)} chord={score.chordShapes?.[i]} harmony={score.harmony?.[i]} selection={cursor.bar===i?cursor:null}/>)}
  </div>
 </div>;
}
