...
const Measure=memo(function Measure({annotationOffsets,chordNameMode,onAnnotationChange,onHarmonyChange,range,pianoStaffLayout,instrument,notes,slurs,chord,chordVisible,harmony,index,keySignature,meter,mobile,incomingTie,selection,lastEntered,onSelect,view,tabRhythm,tabBeamPosition,tabPickingPosition,playPosition,playingRow,allowDrag,placement,layoutEdit,breakBefore,onBreak,systemStart,systemEnd,scoreEnd,systemHeadroom,systemFootroom,engraving,repeatStart,repeatEnd,marker,command,ending,sectionLabel,endBarline,previousEnding,nextEnding,systemNavigation}) {
 ...
 const spacingKey=`${chordVisible}/${engraving.annotationRoom}/${engraving.inset}/${engraving.tickScale}/${engraving.positions?.join(',')}/${sectionLabel}/${endBarline}/${repeatStart}/${repeatEnd}/${marker}/${command}/${ending}/${previousEnding}/${nextEnding}/${systemNavigation}/${tabBeamPosition}/${tabPickingPosition}`;
