...
const Measure=memo(function Measure({onHarmonyEdit,onChordEdit,annotationOffsets,chordNameMode,onAnnotationChange,onHarmonyChange,range,pianoStaffLayout,instrument,notes,slurs,chord,chordVisible,harmony,index,keySignature,meter,mobile,incomingTie,selection,lastEntered,onSelect,view,tabRhythm,tabBeamPosition,tabShortStems,tabPickingPosition,playPosition,playingRow,allowDrag,placement,layoutEdit,breakBefore,onBreak,systemStart,systemEnd,scoreEnd,systemHeadroom,systemFootroom,engraving,repeatStart,repeatEnd,marker,command,ending,sectionLabel,endBarline,previousEnding,nextEnding,systemNavigation}) {
 ...
 const callbacks=useRef(null);callbacks.current={onHarmonyEdit,onChordEdit,onAnnotationChange,onHarmonyChange};
 ...
 useLayoutEffect(()=>{const start=performance.now();try{
  ...
  drawScore(content,{id:`editor-${index}`,title:ko["etudes.editingBar"],bpm:60,instrument,pianoStaffLayout,keySignature,meter,incomingTie,measures:[notes],slurSpans:slurs,repeatMarks:[{repeatStart,repeatEnd,marker,command,ending,sectionLabel,endBarline}],navigationPrevious:{ending:previousEnding},navigationNext:{ending:nextEnding},chordShapes:chord?[chord]:undefined,chordDiagramVisible:[chordVisible],harmony:[harmony],annotationOffsets:[annotationOffsets],chordNameModes:[chordNameMode]},{mobile,editor:true,barOffset:index,tabRhythm,tabBeamPosition,tabShortStems,tabPickingPosition,view,editorWidth,engraving,systemStart,systemEnd,scoreEnd,systemHeadroom,systemFootroom,systemNavigation});const style=document.createElement('style');style.textContent=engravingStyle;root.append(style);bindAnnotationEditing(content.querySelector('svg'),{offsets:annotationOffsets,onMove:(kind,offset)=>callbacks.current.onAnnotationChange?.(index,kind,offset),onNameEdit:()=>callbacks.current.onHarmonyEdit?.(index),onChord:()=>callbacks.current.onChordEdit?.(index),onName:name=>callbacks.current.onHarmonyChange?.(index,name)});setError('');
 }catch(e){setError(formatMessage(ko["etudes.couldNotDisplayThisBarValue"], { value1: e.message }));}
 ...
 useLayoutEffect(()=>{
  ...
  Object.entries({class:'etudePlayingSlot',y1:svg.dataset.playbackTop,y2:svg.dataset.playbackBottom,stroke:'#795536','stroke-opacity':.8,'stroke-width':1.5,'vector-effect':'non-scaling-stroke','pointer-events':'none','aria-label':ko["etudes.rhythmPlaybackPosition"]}).forEach(([k,v])=>marker.setAttribute(k,String(v)));
 ...
 return <section data-pinch-anchor data-bar-index={index} data-layout-row={placement.row} className="etudeEditorMeasure" style={{gridRow:placement.row,gridColumn:'1 / -1',width:`${engraving.cellWidth/engraving.rowWidth*100}%`,marginLeft:`${engraving.cellX/engraving.rowWidth*100}%`,'--layout-tool-overlap':`${20*placement.span/12}px`}} onClick={click}>
  {layoutEdit&&<div className="etudeMeasureLayoutTools"><span>{index+1}</span><button type="button" aria-label={localizeUi(translateUi("etudes.barValue1Value2", { value1: index+1, value2: breakBefore?ko["etudes.mergeWithPreviousLine"]:ko["etudes.splitToNewLine"] }))} title={breakBefore?translateUi("etudes.mergeWithPreviousLine"):translateUi("etudes.splitToNewLine")} disabled={!index} onClick={()=>onBreak(index,!breakBefore)}>{breakBefore?<ArrowUp size={20}/>:<ArrowDown size={20}/>}</button></div>}
  {error&&<p role="alert">{localizeUi(error)}</p>}<div ref={ref}/></section>;
});
export default function EditorScore({onHarmonyEdit,onChordEdit,onAnnotationChange,onHarmonyChange,desktopViewControls,range,onRangeChange,clearRange,capoControl,editControl,score,mobile,cursor,lastEntered,onSelect,onKeyDown,onMove,onMessage,zoom=100,onZoomChange,zoomController,view='both',pageView=false,onLayoutChange,tabRhythm=false,playPosition=null,allowDrag=true}) {
  useLanguage();
 ...
 const widthRatio=!mobile&&(score.instrument==='drums'||perRow===1)?spacing.width/engravingWidth:1;
 ...
 useLayoutEffect(()=>{
  if(!mobile||perRow>1){setFitScale(1);return;}
