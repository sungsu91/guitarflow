...
const Measure=memo(function Measure({range,pianoStaffLayout,instrument,notes,slurs,chord,chordVisible,harmony,index,keySignature,meter,mobile,incomingTie,selection,lastEntered,onSelect,view,tabRhythm,tabBeamPosition,tabPickingPosition,playPosition,playingRow,allowDrag,placement,layoutEdit,breakBefore,onBreak,systemStart,systemEnd,scoreEnd,systemHeadroom,systemFootroom,engraving,repeatStart,repeatEnd,marker,command,ending,sectionLabel,endBarline,previousEnding,nextEnding,systemNavigation}) {
 ...
 useLayoutEffect(()=>{const start=performance.now();try{
  ...
  drawScore(content,{id:`editor-${index}`,title:'편집 마디',bpm:60,instrument,pianoStaffLayout,keySignature,meter,incomingTie,measures:[notes],slurSpans:slurs,repeatMarks:[{repeatStart,repeatEnd,marker,command,ending,sectionLabel,endBarline}],navigationPrevious:{ending:previousEnding},navigationNext:{ending:nextEnding},chordShapes:chord?[chord]:undefined,chordDiagramVisible:[chordVisible],harmony:[harmony]},{mobile,editor:true,barOffset:index,tabRhythm,tabBeamPosition,tabPickingPosition,view,editorWidth,engraving,systemStart,systemEnd,scoreEnd,systemHeadroom,systemFootroom,systemNavigation});const style=document.createElement('style');style.textContent=engravingStyle;root.append(style);setError('');
 ...
 },[pianoStaffLayout,instrument,notes,slurs,chord,harmony,index,keySignature,meter,mobile,incomingTie,tabRhythm,view,editorWidth,systemStart,systemEnd,scoreEnd,systemHeadroom,systemFootroom,spacingKey]);
 useLayoutEffect(()=>{
...
export default function EditorScore({desktopViewControls,range,onRangeChange,clearRange,capoControl,editControl,score,mobile,cursor,lastEntered,onSelect,onKeyDown,onMove,onMessage,zoom=100,onZoomChange,zoomController,view='both',pageView=false,onLayoutChange,tabRhythm=false,playPosition=null,allowDrag=true}) {
