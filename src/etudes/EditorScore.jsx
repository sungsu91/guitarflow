...
const Measure=memo(function Measure({instrument,notes,chord,harmony,index,keySignature,meter,mobile,incomingTie,selection,beamSelection,onSelect,view,tabRhythm,tabBeamPosition,tabPickingPosition,playPosition,playingRow,allowDrag,placement,layoutEdit,breakBefore,onBreak,systemStart,systemEnd,scoreEnd,systemHeadroom,engraving,repeatStart,repeatEnd,marker,command,ending,previousEnding,nextEnding,systemNavigation}) {
 ...
 const spacingKey=`${engraving.inset}/${engraving.tickScale}/${repeatStart}/${repeatEnd}/${marker}/${command}/${ending}/${previousEnding}/${nextEnding}/${systemNavigation}/${tabBeamPosition}/${tabPickingPosition}`;
 ...
 useLayoutEffect(()=>{const start=performance.now();try{
  ...
  drawScore(content,{id:`editor-${index}`,title:'편집 마디',bpm:60,instrument,keySignature,meter,incomingTie,measures:[notes],repeatMarks:[{repeatStart,repeatEnd,marker,command,ending}],navigationPrevious:{ending:previousEnding},navigationNext:{ending:nextEnding},chordShapes:chord?[chord]:undefined,harmony:[harmony]},{mobile,editor:true,barOffset:index,tabRhythm,tabBeamPosition,tabPickingPosition,view,editorWidth,engraving,systemStart,systemEnd,scoreEnd,systemHeadroom,systemNavigation});const style=document.createElement('style');style.textContent=engravingStyle;root.append(style);setError('');
 ...
 },[instrument,notes,chord,harmony,index,keySignature,meter,mobile,incomingTie,tabRhythm,view,editorWidth,systemStart,systemEnd,scoreEnd,systemHeadroom,spacingKey]);
 useLayoutEffect(()=>{
  ...
  if(selection.mode==='staff'){const p=pitchForMidi(selection.midi??60,keySignature),step=staffStepForPitch(p,instrument);y=Number(el.dataset.staffBottom)-step*5-5;height=10;}
 ...
 },[instrument,notes,selection,keySignature,view,editorWidth,systemStart,systemEnd,scoreEnd,systemHeadroom,spacingKey]);
 useLayoutEffect(()=>{
 ...
 },[beamSelection,notes,view,editorWidth,systemStart,systemEnd,scoreEnd,systemHeadroom,spacingKey]);
 useLayoutEffect(()=>{ref.current.dataset.drag=String(allowDrag);},[allowDrag]);
 useLayoutEffect(()=>{ref.current.dataset.view=view;ref.current.shadowRoot?.querySelector('svg')?.setAttribute('aria-label',view==='tab'?'TAB 악보':view==='staff'?'오선보 악보':'오선보와 TAB 악보');},[view,editorWidth,systemStart,systemEnd,scoreEnd,systemHeadroom,spacingKey]);
 ...
 },[playingRow,notes,tabRhythm,view,editorWidth,systemStart,systemEnd,scoreEnd,systemHeadroom,spacingKey]);
 useLayoutEffect(()=>{
 ...
 },[playPosition,notes,meter,tabRhythm,view,editorWidth,systemStart,systemEnd,scoreEnd,systemHeadroom,spacingKey]);
 const click=e=>{if(e.target.closest('.etudeMeasureLayoutTools'))return;const hit=ref.current.shadowRoot?.elementFromPoint(e.clientX,e.clientY)?.closest?.('[data-event]')??e.nativeEvent.composedPath().find(node=>node.dataset?.event!==undefined);if(!hit){onSelect({bar:index,event:0,string:selection?.string??scoreInstrument(instrument).tuning.length,mode:'tab',target:'bar'});e.currentTarget.closest('[data-score-input]')?.focus({preventScroll:true});return;}const mode=hit.dataset.mode;let midi;
  if(mode==='staff'){const svg=hit.ownerSVGElement,p=svg.createSVGPoint();p.x=e.clientX;p.y=e.clientY;const local=p.matrixTransform(svg.getScreenCTM().inverse());const step=Math.round((Number(hit.dataset.staffBottom)-local.y)/5);midi=midiAtStaffStep(step,keySignature,instrument);}
  onSelect({bar:index,event:Number(hit.dataset.event),string:hit.classList.contains('etudePickHit')?(selection?.string??scoreInstrument(instrument).tuning.length):Number(hit.dataset.string??1),mode,midi:hit.dataset.midi!==undefined?Number(hit.dataset.midi):midi});e.currentTarget.closest('[data-score-input]')?.focus({preventScroll:true});
 };
...
export default function EditorScore({score,mobile,cursor,beamSelection,beamRangeMode,onBeamRange,onSelect,onKeyDown,onMove,onMessage,zoom=100,onZoomChange,zoomController,view='both',pageView=false,onLayoutChange,tabRhythm=false,playPosition=null,allowDrag=true}) {
 ...
 const spacing=useMemo(()=>scoreSpacing(score,{placements,view,width:engravingWidth}),[score,placements,view,engravingWidth]);
 // Dense input may need a wider measure, but must not shrink every glyph.
 // Keep one-measure editing at a stable scale; overflow stays in the score.
 const widthRatio=perRow===1?spacing.width/engravingWidth:1;
 ...
 useLayoutEffect(()=>{
  ...
  const fit=()=>{
   ...
   for(const host of canvas.querySelectorAll('[data-draw-count]')){
    ...
    if(box.width&&cell)tallest=Math.max(tallest,canvas.clientWidth*widthRatio*cell.cellWidth/cell.rowWidth*box.height/box.width);
   }
