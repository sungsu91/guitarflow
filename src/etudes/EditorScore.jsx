...
const Measure=memo(function Measure({instrument,notes,slurs,chord,chordVisible,harmony,index,keySignature,meter,mobile,incomingTie,selection,lastEntered,onSelect,view,tabRhythm,tabBeamPosition,tabPickingPosition,playPosition,playingRow,allowDrag,placement,layoutEdit,breakBefore,onBreak,systemStart,systemEnd,scoreEnd,systemHeadroom,systemFootroom,engraving,repeatStart,repeatEnd,marker,command,ending,sectionLabel,endBarline,previousEnding,nextEnding,systemNavigation}) {
 ...
 const spacingKey=`${chordVisible}/${engraving.inset}/${engraving.tickScale}/${engraving.positions?.join(',')}/${sectionLabel}/${endBarline}/${repeatStart}/${repeatEnd}/${marker}/${command}/${ending}/${previousEnding}/${nextEnding}/${systemNavigation}/${tabBeamPosition}/${tabPickingPosition}`;
 ...
 useLayoutEffect(()=>{const start=performance.now();try{
  ...
  drawScore(content,{id:`editor-${index}`,title:'편집 마디',bpm:60,instrument,keySignature,meter,incomingTie,measures:[notes],slurSpans:slurs,repeatMarks:[{repeatStart,repeatEnd,marker,command,ending,sectionLabel,endBarline}],navigationPrevious:{ending:previousEnding},navigationNext:{ending:nextEnding},chordShapes:chord?[chord]:undefined,chordDiagramVisible:[chordVisible],harmony:[harmony]},{mobile,editor:true,barOffset:index,tabRhythm,tabBeamPosition,tabPickingPosition,view,editorWidth,engraving,systemStart,systemEnd,scoreEnd,systemHeadroom,systemFootroom,systemNavigation});const style=document.createElement('style');style.textContent=engravingStyle;root.append(style);setError('');
 ...
 },[instrument,notes,slurs,chord,harmony,index,keySignature,meter,mobile,incomingTie,tabRhythm,view,editorWidth,systemStart,systemEnd,scoreEnd,systemHeadroom,systemFootroom,spacingKey]);
 useLayoutEffect(()=>{
  ...
  let y=Number(el.dataset.cursorY),height=14;if(instrument==='drums'&&!selection.noteId){y=Number(el.dataset.staffBottom)-48;height=56;}
  ...
  const marker=document.createElementNS('http://www.w3.org/2000/svg','rect');Object.entries({class:'etudeInputCursor is-cursor',x:instrument==='drums'&&!selection.noteId?Number(el.dataset.inputCenterX)-1:compactCursor?center-cursorWidth/2:Number(el.dataset.cursorX)-2,y,width:instrument==='drums'&&!selection.noteId?2:cursorWidth,height,rx:instrument==='drums'?0:compactCursor?1:3,fill:instrument==='drums'&&!selection.noteId?'#bd8521':'#167254','fill-opacity':instrument==='drums'&&!selection.noteId?1:0.16,stroke:compactCursor?'#167254':'none','stroke-width':compactCursor?.7:0,'vector-effect':'non-scaling-stroke','aria-label':'현재 입력 위치','pointer-events':'none'}).forEach(([k,v])=>marker.setAttribute(k,String(v)));el.ownerSVGElement.prepend(marker);
 ...
 const click=e=>{if(e.target.closest('.etudeMeasureLayoutTools'))return;const hit=ref.current.shadowRoot?.elementFromPoint(e.clientX,e.clientY)?.closest?.('[data-event]')??e.nativeEvent.composedPath().find(node=>node.dataset?.event!==undefined);if(!hit){onSelect({bar:index,event:0,string:selection?.string??scoreInstrument(instrument).tuning.length,mode:'tab',target:'bar'});e.currentTarget.closest('[data-score-input]')?.focus({preventScroll:true});return;}const mode=hit.dataset.mode;let midi;
  ...
  onSelect({bar:index,event:Number(hit.dataset.event),string:hit.classList.contains('etudePickHit')?(selection?.string??scoreInstrument(instrument).tuning.length):Number(hit.dataset.string)||1,noteId:hit.dataset.toneId,lowerRest:hit.dataset.lowerRest==='true',hand:hit.dataset.hand,mode,midi:hit.dataset.toneId||instrument==='drums'?Number(hit.dataset.midi):midi});e.currentTarget.closest('[data-score-input]')?.focus({preventScroll:true});
 };
...
export default function EditorScore({capoControl,editControl,score,mobile,cursor,lastEntered,onSelect,onKeyDown,onMove,onMessage,zoom=100,onZoomChange,zoomController,view='both',pageView=false,onLayoutChange,tabRhythm=false,playPosition=null,allowDrag=true}) {
