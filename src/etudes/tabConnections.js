...
export function tabArcGeometry(first,last,firstIndex=0,lastIndex=0){
 ...
 const width=Math.max(0,x2-x1),lift=Math.min(32,Math.max(12,width*.24));
 const cx1=x1+width/3,cx2=x2-width/3;
 return {x1,x2,y1,y2,lift,cx1,cx2,d:'M '+x1+' '+y1+' C '+cx1+' '+(y1-lift)+' '+cx2+' '+(y2-lift)+' '+x2+' '+y2};
}
export class ReadableTabTie{
 ...
 (n.first_indices??n.last_indices??[0]).forEach((index,j)=>{const a=tabArcGeometry(n.first_note,n.last_note,index,n.last_indices?.[j]??index);c.beginPath();c.moveTo(a.x1,a.y1);c.bezierCurveTo(a.cx1,a.y1-a.lift,a.cx2,a.y2-a.lift,a.x2,a.y2);c.stroke();});
...
export function drawReadableSlide(context,first,last,pair){
 ...
 return {x1:center-gap/2,y1:first.getYs()[pair.first]-dy,x2:center+gap/2,y2:last.getYs()[pair.last]+dy};
}
