...
export default function EditorAudioDock({fixedInstrument=null,compact=false,sound,onSound,playing,disabled,onPlay,bpm,onBpm,audible,onAudible,status,error}){
 ...
 return <section className={`editorAudioDock ${compact?'is-one-row':''}`} aria-label="음색과 재생">
  ...
  {compact&&fixedInstrument!=='piano'&&<label className="editorSoundToggle"><input type="checkbox" checked={sound} onChange={onSound}/><span>악보 소리</span></label>}
