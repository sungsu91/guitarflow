...
export default function EditorAudioDock({sound,onSound,instrument,onInstrument,playing,disabled,onPlay,bpm,onBpm,audible,onAudible,status,error}){
 ...
 return <section className="editorAudioDock" aria-label="음색과 재생">
  <div className="editorTimbre"><label className="editorSoundToggle"><input type="checkbox" checked={sound} onChange={onSound}/>악보 소리</label><div role="group" aria-label="음색">{[['clean-guitar','클린 기타'],['piano','피아노']].map(([id,label])=><button type="button" key={id} disabled={!sound} aria-pressed={sound&&instrument===id} onClick={()=>onInstrument(id)}>{label}</button>)}</div></div>
