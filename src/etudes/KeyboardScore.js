...
import {drumForMidi,pianoStaffLayout} from './scoreInstruments.js';
...
export function drawKeyboardScore(element,score,{mobile=false,editor=false,editorWidth=980,barOffset=0,systemStart=true,systemEnd=true,measuresPerRow=1}={}){
 const drums=score.instrument==='drums',layout=pianoStaffLayout(score.pianoStaffLayout??score.document?.viewSettings?.pianoStaffLayout),single=!drums&&layout!=='grand';
 ...
 const singleAbove=single?Math.max(0,...pitches.map(t=>(step(t,layout==='bass')-12)*5)):0,singleBelow=single?Math.max(0,...pitches.map(t=>(-4-step(t,layout==='bass'))*5)):0;
 const above=drums?0:single?singleAbove:Math.max(0,...right.map(t=>(step(t,false)-12)*5)),between=drums||single?0:Math.max(0,...right.map(t=>(-4-step(t,false))*5))+Math.max(0,...left.map(t=>(step(t,true)-12)*5)),below=drums?0:single?singleBelow:Math.max(0,...left.map(t=>(-4-step(t,true))*5));
 const rowHeight=(drums||single?200:mobile?245:310)+above+between+below,perRow=editor?1:(measuresPerRow||score.document?.viewSettings?.measuresPerRow||1),width=Math.max(editorWidth,220*perRow),height=Math.ceil(score.measures.length/perRow)*rowHeight;
 element.replaceChildren();const renderer=new Renderer(element,Renderer.Backends.SVG);renderer.resize(width,height);const context=renderer.getContext(),svg=element.querySelector('svg');svg.setAttribute('viewBox',`0 0 ${width} ${height}`);svg.style.width='100%';svg.style.height='auto';svg.dataset.notationView='staff';svg.dataset.pianoStaffLayout=drums?'percussion':layout;svg.dataset.playbackTop='30';svg.setAttribute('role','img');svg.dataset.playbackBottom=String(rowHeight-25);
 ...
 score.measures.forEach((events,b)=>{
  ...
  const staves=(drums?['percussion']:single?[layout]:['treble','bass']).map((clef,hand)=>{
  ...
  const groups=(drums||single?[staves[0],staves[0]]:staves).map((stave,hand)=>{
   ...
   const notes=voiceEvents.map((e,i)=>{const rest=!tones[i].length;if(single&&e.blank){return new GhostNote({duration:e.duration+(e.dotted?'d':'')});}if(drums&&rest&&!(hand===1&&e.lowerRest)&&(hand===1||!e.rest)){const ghost=new GhostNote({duration:e.duration+(e.dotted?'d':'')});ghost.getStemDirection=()=>hand?-1:1;ghost.getStemExtents=()=>({topY:stave.getYForLine(2),baseY:stave.getYForLine(2)});return ghost;}const n=new StaveNote({clef:drums?'percussion':single?layout:hand?'bass':'treble',keys:rest?[drums&&hand?'d/4':single?(layout==='bass'?(hand?'f/3':'a/3'):(hand?'d/5':'b/4')):hand?'d/3':'b/4']:tones[i].map(t=>drums?drumForMidi(t.midi).key:t.pitch.key),duration:e.duration+(e.dotted?'d':'')+(rest?'r':''),auto_stem:!drums&&!single,...(drums||single?{stem_direction:hand?-1:1}:{})});if(e.dotted)Dot.buildAndAttach([n],{all:true});return n;});
   ...
   const voice=new Voice({num_beats:meter[0],beat_value:meter[1]}).setMode(Voice.Mode.SOFT).addTickables(notes);if(!drums&&!single)Accidental.applyAccidentals([voice],score.keySignature);
   let beams=Beam.generateBeams(notes,{groups:Beam.getDefaultBeamGroups(meter.join('/')),...(drums||single?{stem_direction:hand?-1:1}:{})});
   return {notes,tones,voice,tuplets,beams,stave,indices,voiceEvents};
  });
  if(single)Accidental.applyAccidentals(groups.map(g=>g.voice),score.keySignature);
  ...
  groups.forEach(({voice,stave,notes,tones,tuplets,beams,indices,voiceEvents},hand)=>{
   ...
   voiceEvents.forEach((e,i)=>{const incomingTie=typeof score.incomingTie==='string'?score.incomingTie.split('|').includes(e.id):score.incomingTie;
    ...
    const cx=notes[i].getAbsoluteX(),cy=stave.getYForLine(2),base={'data-event':indices[i],'data-mode':'staff','data-staff-bottom':stave.getYForLine(4),'data-cursor-x':cx-12,'data-input-center-x':cx+notes[i].getGlyphWidth()/2,'data-cursor-y':cy-7,'data-clef':drums?'percussion':single?layout:hand?'bass':'treble','data-hand':hand?'left':'right','data-lower-rest':Boolean(drums&&hand===1&&e.lowerRest&&!tones[i].length),class:'etudeEditorHit',fill:'transparent'};
  ...
  // Shared-staff voices keep background slots below every tone handle.
  if(editor&&(drums||single))svg.querySelectorAll('.etudeNoteHandle').forEach(handle=>svg.append(handle));
