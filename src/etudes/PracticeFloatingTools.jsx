...
function EtudeRemote({controls:c,model,mobile,onHeight}){
 ...
 {id:'subdivision',label:ko["app.subdivision"],value:model.subdivision,options:METRONOME_SUBDIVISION_OPTIONS,onChange:model.setSubdivision},
 {id:'tone',label:ko["app.sound"],tone:true,value:model.tone,options:METRONOME_TONE_OPTIONS,onChange:model.setTone},
 ]}/><PracticeRepeatControls model={model}/><FollowPatternControl value={model.followMode} onChange={model.setFollowMode}/>{!mobile&&<label className="etudeOptionalSoundToggle"><input type="checkbox" checked={Boolean(c.sound&&model.followMode!=='off')} disabled={model.followMode==='off'} onChange={c.onSound}/><span><Translation id="etudes.scoreSound" /></span></label>}</> }</PracticePopover>}{c.error&&<p role="alert">{localizeUi(c.error)}</p>}
 </section>;
...
function MovableBackingPanel({scope,close,children,origin,mobile,clearance,defaultOrigin}){
  useLanguage();
 const position=useFloatingPosition('riff-'+scope+'-backing-panel-position',false,false,false,false,origin,defaultOrigin||(!mobile?{x:innerWidth-464,y:100}:null),false,clearance);
...
function BackingSurface(props) {
  return props.controller.sharedPlayback ? <SharedBackingTrigger {...props}/> : <LocalBackingSurface {...props}/>;
}
...
function LocalBackingSurface({controller,children,open,setOpen,scope,triggerTarget,mobile,clearance}) {
  useLanguage();
  ...
  const sharedEdgeActive=controller.sharedPlayback&&(controller.isPlaying||controller.isPaused);
  const floating=useFloatingPosition(`riff-${scope}-backing-position`,true,false,false,false,null,null,false,0,Boolean(triggerTarget)||!sharedEdgeActive);
  ...
  return <>
    {triggerTarget?createPortal(<span ref={floating.ref} className="etudeBackingToggle"><button ref={trigger} type="button" data-ui="backing-loop" aria-label={translateUi("etudes.backingLoop")} aria-pressed={open||minimized} aria-expanded={open} aria-controls="etude-backing-panel" onClick={()=>{if(open){close();}else{setMinimized(false);setOpen(true);}}}><AudioLines aria-hidden="true"/><Translation id="score.backingLoopCompact" />{controller.isPlaying&&<i role="status" aria-label={translateUi("etudes.backingLoopPlaying")}> ·</i>}</button></span>,triggerTarget):(!sharedEdgeActive&&<div ref={floating.ref} className="etudeBackingHandle" style={floating.style}><button ref={trigger} type="button" aria-label={open?translateUi("etudes.collapseBackingLoopPanel"):translateUi("etudes.expandBackingLoopPanel")} aria-expanded={open} aria-controls="etude-backing-panel" {...floating.handle} onClick={e=>{if(open)close();else{const r=e.currentTarget.getBoundingClientRect();setOpenOrigin({x:r.left,y:r.top});setOpen(true);}}}><span aria-hidden="true">{open?'›':'‹'}</span>{controller.isPlaying&&<i role="status" aria-label={translateUi("etudes.backingLoopPlaying")}/>}</button></div>)}
    {triggerTarget&&minimized&&!open&&!sharedEdgeActive&&<MovableEdgeTab scope={scope} kind="backing" playing={controller.isPlaying} onOpen={e=>{const r=e.currentTarget.getBoundingClientRect();setOpenOrigin({x:r.left,y:r.top});setMinimized(false);setOpen(true);}}/>}
    {open&&<MovableBackingPanel scope={scope} close={close} origin={openOrigin} mobile={mobile} clearance={clearance} defaultOrigin={!mobile&&trigger.current?{x:innerWidth-464,y:trigger.current.getBoundingClientRect().bottom+12}:null}>{children}</MovableBackingPanel>}
  </>;
}
...
export function PracticeBackingPanel({mobile,scope='etude',open:controlledOpen,onOpenChange,clearance=0,triggerTarget=null}){
 ...
 return <div className="etudeFloatingTheme" style={{'--etude-metro-height':Math.max(clearance,navClearance)+'px'}}><BackingLoop mobile={mobile} desktopPresentation="standalone" ownerMode="etudes" renderSurface={(controller,content)=><BackingSurface clearance={clearance} mobile={mobile} controller={controller} open={open} setOpen={setOpen} scope={scope} triggerTarget={triggerTarget}>{content}</BackingSurface>}/></div>;
}
