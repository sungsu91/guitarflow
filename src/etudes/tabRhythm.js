...
export function drawTabRhythm(svg,events,tabs,tab,beamGeometry) {
 ...
 for(const plan of beamGeometry){
  ...
  plan.levels.forEach((segments,level)=>segments.forEach(({start,end})=>{
   ...
   for(let j=0;j<stops.length-1;j++)line(project(stops[j]),base-level*6,project(stops[j+1]),base-level*6,4,full?'tabRhythmBeam':'tabRhythmFlag',plan.indices[nearest(stops[j])]);
  }));
 }
 events.forEach((e,i)=>{
  if(e.rest||Number(e.duration)<8||grouped.has(i))return;
  for(let level=0;level<(e.duration==='16'?2:1);level++)line(x(i),base-level*6,x(i)+8,base-level*6-3,3,'tabRhythmFlag',i);
 });
 for(const group of tupletGroups(events)){
  if(group.every(i=>isBlankEvent(events[i])))continue;
  const left=x(group[0])-5,right=x(group.at(-1))+5,center=(left+right)/2,y=base+19;
  ...
  line(left,y,left,y-4,1,'tabTupletBracket');line(left,y,center-8,y,1,'tabTupletBracket');line(center+8,y,right,y,1,'tabTupletBracket');line(right,y,right,y-4,1,'tabTupletBracket');
 }
 return base;
}
