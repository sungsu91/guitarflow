...
export default function ScoreSaveDialog({document,onSave,onClose,mode='save'}){
 ...
 return <dialog ref={ref} className="scoreSaveDialog" aria-label={editing?"악보 설정":"악보 저장 정보"} onKeyDown={e=>e.stopPropagation()} onCancel={e=>{e.preventDefault();e.stopPropagation();onClose();}}>
  <form onSubmit={submit}><h2>{editing?"악보 설정":"악보 저장"}</h2><p>{editing?"현재 작업에 적용합니다. 최종 저장은 상단 저장 버튼을 눌러주세요.":"제목과 곡 정보가 악보 첫 페이지 상단에 표시됩니다."}</p>
   ...
   {document.instrument==='piano'?<label>피아노 보표 구성<select aria-label="피아노 보표 구성" value={pianoLayout} onChange={e=>setPianoLayout(e.target.value)}>{PIANO_STAFF_LAYOUTS.map(o=><option key={o.value} value={o.value}>{o.label}</option>)}</select></label>:<label>보표 보기<select aria-label="보표 보기" value={view} onChange={e=>setView(e.target.value)}>{isFretted(document.instrument)&&<><option value="tab">TAB만</option><option value="both">오선보+TAB</option></>}<option value="staff">오선보만</option></select></label>}
