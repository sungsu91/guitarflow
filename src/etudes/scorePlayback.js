...
export function scoreTimeline(score, bpm = score.bpm) {
  ...
  for (const [bar, measure] of score.measures.entries()) {
    ...
    for (const e of measure) {
      ...
      if (!e.rest) for (const [toneIndex,tone] of tones.entries()) {
        ...
        const note = prior ?? {id:e.id, bar, pickStroke:e.pickStroke??null, vibrato:Boolean(e.vibrato), harmonic:Boolean(tone.harmonic), dead:Boolean(tone.dead??e.dead), midi:tone.midi, fret:tone.fret, string:tone.string, start:toneStart, duration:0, technique:null};
  ...
  return {events:events.sort((a,b)=>a.start-b.start), duration:writtenEnd};
}
...
export function guitarVoiceTimeline(score, bpm = score.bpm) {
  ...
  const rests=score.measures.flatMap((measure,bar)=>measure.filter(e=>e.rest&&!e.blank).map(e=>(bar*capacity+e.onset)/480*60/bpm));
