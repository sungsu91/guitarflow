...
function KeyControls({draft,setDraft}){
 ...
 return <section className="editorKeyControls" aria-label="조성 설정"><strong>조성</strong><div className="editorKeyModes" role="group" aria-label="장조 또는 단조"><button type="button" aria-pressed={!minor} onClick={()=>choose(pair[0])}>장조</button><button type="button" aria-pressed={minor} onClick={()=>choose(pair[1])}>단조</button></div><EditorChoiceMenu label="조성 선택" value={draft.keySignature} onChange={choose} options={KEY_PAIRS.map(p=>({value:p[column],label:`${p[column].replace(/m$/,'')} ${minor?'단조':'장조'}`}))}/><small>{pair[2]===0?'조표 없음':`${pair[2]>0?'♯':'♭'} ${Math.abs(pair[2])}개`}</small></section>;
}
...
export default function useEditorSettings(draft,setDraft,midi,mobile=false){
 ...
 const tuning=isFretted(draft.instrument)&&<button type="button" className="editorTuningButton" aria-expanded={menu==='tuning'} aria-haspopup="dialog" onClick={e=>open('tuning',e)}>{tuningName(draft)} ▾</button>;
 const capo=isFretted(draft.instrument)&&<button type="button" aria-expanded={menu==='capo'} aria-haspopup="dialog" onClick={e=>open('capo',e)}>{mobile?`${draft.keySignature} · `:''}{draft.capo?`카포 ${draft.capo}`:'카포 없음'} ▾</button>;
 ...
 const popup=menu&&<EditorPopover anchor={anchor.current} onClose={close} label={menu==='tuning'?'튜닝 설정':menu==='capo'?(mobile?'조성·카포 설정':'카포 설정'):'편집 메뉴'}>
  <header><strong>{menu==='tuning'?'튜닝':menu==='capo'?(mobile?'조성 · 카포':'카포'):'편집'}</strong><button type="button" aria-label="편집 설정 닫기" onClick={()=>close(true)}>×</button></header>
  {pending?<><fieldset><legend>기존 음표 변경 방식</legend>{[['pitch','음높이 유지'],['fingering','운지 유지']].map(([id,label])=><label key={id}><input type="radio" name="tuning-change-mode" checked={mode===id} onChange={()=>setMode(id)}/>{label}</label>)}</fieldset><p>{mode==='pitch'?'실제 음높이를 유지하고 새 카포·튜닝에 맞는 TAB 위치를 계산합니다. 아래 운지 변경을 확인한 뒤 적용하세요.':'현과 카포 기준 프렛을 유지하고 실제 음높이를 바꿉니다.'}</p>{preview?.conflicts?.length>0&&<p role="alert">직접 지정 운지 {preview.conflicts.length}개 재계산: {preview.conflicts.slice(0,4).join(', ')}. 적용하면 해당 현·프렛이 변경됩니다.</p>}{preview?.unplaced>0&&<p role="status">TAB 배치 불가 {preview.unplaced}음 · 원래 음높이 보존</p>}{preview?.error&&<p role="alert">{preview.error}</p>}<button type="button" disabled={!preview||Boolean(preview.error)} onClick={()=>{setDraft(preview.document);close(true);}}>{mode==='pitch'&&preview?.conflicts?.length?'운지 재계산 후 적용':'변경 적용'}</button><button type="button" onClick={()=>setPending(null)}>취소</button></>:
   ...
   menu==='capo'?<>{mobile&&<KeyControls draft={draft} setDraft={setDraft}/>}<label className="editorCapoSelect">카포<EditorChoiceMenu label="카포 선택" value={draft.capo??0} onChange={capo=>{if(capo!==(draft.capo??0))request({capo});}} options={Array.from({length:Math.min(12,maxFret(draft))+1},(_,n)=>({value:n,label:n?`${n}프렛`:'없음'}))}/></label><p>TAB 0 = 카포 개방현 · 실제 위치 최대 {maxFret(draft)}프렛</p></>:
