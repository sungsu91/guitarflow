...
export default function ScorePlayback({score,bpm=score?.bpm,disabled=false,practice=false,toolbar=false,toolbarTarget=null,compact=false,dock=false,onPosition,controller,startAt={bar:0,event:0},onBpm,onBeforePlay,renderPractice,repeatCount=1,metroOptions={}}) {
 ...
 const play=async(from=startAt)=>{
  ...
  try{
   ...
   const timeline=optionalSound?scoreTimeline(score,bpm,false,{playEmptyScore:dock||practice}):guitarVoiceTimeline(score,bpm),capacity=score.meter[0]*1920/score.meter[1],slots=playbackSlots(score,timeline.order),offset=seekTick(slots,from)/480*60/bpm;
   ...
   const clicks=practiceClicks(performedMeasures(score,timeline.order)).flatMap(c=>Array.from({length:metroOptions.clicksPerBeat??1},(_,i)=>({...c,tick:c.tick+i*1920/c.meter[1]/(metroOptions.clicksPerBeat??1),downbeat:c.downbeat&&i===0}))).map(c=>({...c,time:c.tick*60/bpm/480}));
   ...
   const tick=()=>{
    ...
    if(practice&&currentSlot){const beat=ctx.currentTime<clock.origin?-1:Math.min(currentSlot.meter[0]-1,Math.floor((s.getCycleTick()-currentSlot.barStart)/(1920/currentSlot.meter[1])));setPulse(p=>p?.beat===beat&&p?.meter.join()===currentSlot.meter.join()?p:{beat,meter:currentSlot.meter});}
    const cycle=Math.floor(s.getTimelineTick()/cycleTicks);
    if(currentSlot&&(currentSlot!==s.slot||cycle!==s.cycle)){s.slot=currentSlot;s.cycle=cycle;publish({...currentSlot,playing:true,getTimelineTick:s.getCycleTick,getCurrentSlot:()=>slotAtTick(slots,s.getCycleTick()),getBarTick:()=>Math.max(0,Math.min(currentSlot.capacity,s.getCycleTick()-currentSlot.barStart))});}
 ...
 const updateSound=s=>{
  ...
  s.soundReady=false;s.voices=[];s.nextVoices=[];s.index=0;s.output.dispose();s.output=createScoreVoiceOutput(s.ctx);
