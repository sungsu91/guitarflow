...
export function pitchForMidi(midi,keySignature='C',spelling) {
...
function compileBar(bar,d) {
 const context=JSON.stringify([d.tuning,d.keySignature,d.meter]);const found=cache.get(bar);if(found?.context===context)return found.result;
 compileStats.bars++;
 ...
 if(bar.chord?.barre){const b=bar.chord.barre;if(!Number.isInteger(b.fret)||b.fret<1||b.fret>24||!Number.isInteger(b.from)||!Number.isInteger(b.to)||b.from>6||b.to<1||b.from<=b.to)errors.push('코드표 바레의 프렛과 시작·끝 줄을 확인하세요.');}
...
export function compileDocumentV2(d,base={}) {
 ...
 if(!Array.isArray(d.tuning)||d.tuning.length!==6||d.tuning.some(v=>!Number.isInteger(v)||v<24||v>88))errors.push('튜닝은 1→6번줄의 MIDI 정수 6개입니다.');
