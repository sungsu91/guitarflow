...
import {normalizePitches,effectiveTuning,soundingMidi,tuningCaption,maxFret} from './scoreTuning.js';
...
export default function ScoreEditor({score,document:initialDocument,original,mobile,onClose,onSave,onImportPdf,compactImport=false}) {
 const [draft,updateDraft]=useState(()=>normalizePitches(initialDocument??toScoreDocument(score)));
 ...
 const view=draft.viewSettings?.notationView??'tab';
 ...
 const setDraft=useCallback((update,{coalesce=false,keepDigits=false}={})=>{if(!keepDigits)cancelDigits();const before=draftRef.current;let after=typeof update==='function'?update(before):update;after=shareUnchanged(before,normalizePitches(after));if(before===after)return;
 ...
 const event=draft.measures[barIndex].events[eventIndex],tone=event.notes.find(n=>n.id===cursor.noteId)??event.notes.find(n=>n.string===cursor.string)??event.notes[0];
 const select=useCallback((c,{seekPlayback=true}={})=>{if(c.string!==cursorRef.current.string||!draftRef.current.measures[c.bar]?.events[c.event]?.notes.some(n=>n.id===c.noteId))c={...c,noteId:undefined};c={...c,string:Math.min(c.string,draftRef.current.tuning.length)};setBeamSelection(null);cancelDigits();cursorRef.current=c;setCursor(c);setCandidates([]);setPropertySection(c.target==='bar'?'bar':'note');const selected=draftRef.current.measures[c.bar]?.events[c.event];if(selected&&(!selected.blank||selected.tuplet)&&dottedMode==='off'&&!tripletInput)setSelectedDuration(selected.duration);if(seekPlayback)playbackController.current?.seek(c);if(c.midi!==undefined)setPitch(c.midi);},[cancelDigits,dottedMode,tripletInput]);
 ...
 const midi=useMidiKeyboard(pitches=>{
  try{const at=cursorRef.current,changed=enterMidiNotes(draftRef.current,at,pitches,midiRhythm.current);
   ...
   const next=nextEntry(changed.document,at);setDraft(next.document);setDottedMode(changed.dottedMode);setTupletMode(changed.tupletMode);setTupletSession(changed.completed?null:changed.session);setRestMode(false);select(next.cursor,{seekPlayback:false});
 ...
 },()=>Boolean(dialog.current?.open&&!settings.isOpen&&!mobileSheet&&!toolSection&&!resetOpen&&!pickingOpen&&!saveRequest&&!closing&&!instrumentChange&&!playbackController.current?.isPlaying()&&document.activeElement?.closest('[data-score-input]')&&document.activeElement?.closest('dialog')===dialog.current&&cursorRef.current.target!=='bar'&&document.visibilityState==='visible'),()=>`${draftRef.current.id}:${cursorRef.current.bar}:${cursorRef.current.event}`);
