...
import {enterFret,enterFretWithDuration,setEventDuration,resolveFretInput,deleteTone,moveFingering,setRest,insertEvent,moveTone,splitEvent,applyPicking,nextEntry,cursorStep,copyBars,pasteBars} from './editorCommands.js';
...
export default function ScoreEditor({score,document:initialDocument,original,mobile,onClose,onSave,onImportPdf,compactImport=false}) {
 ...
 const [twoDigit,setTwoDigit]=useState(false),[view,setView]=useState('both'),[panelVisible,setPanelVisible]=useState(true),[tabRhythm,setTabRhythm]=useState(false),[mobileSheet,setMobileSheet]=useState(null),[mobileDrag,setMobileDrag]=useState(false),[playPosition,setPlayPosition]=useState(null);
 ...
 const select=useCallback(c=>{cancelDigits();cursorRef.current=c;setCursor(c);setCandidates([]);setPropertySection(c.target==='bar'?'bar':'note');playbackController.current?.seek(c);if(c.midi!==undefined)setPitch(c.midi);},[cancelDigits]);
 ...
 const advance=(from=cursorRef.current)=>{if(autoAdvance){const next=stepEntry(from);select({...next,target:undefined});if(next.bar===from.bar&&next.event===from.event)setMessage('마디의 길이를 확인하세요. 최대 64마디까지 입력할 수 있습니다.');}};
 const scheduleDigitExpiry=()=>{digitTimer.current=setTimeout(()=>{digits.current=null;digitTimer.current=null;},420);};
 ...
 const keyDown=(e,tabInput=false)=>{
  ...
  if(/^\d$/.test(k)){
   ...
   const previous=digits.current,next=resolveFretInput(previous,k,`${at.bar}:${at.event}:${at.string}`,performance.now(),twoDigit||(mobile&&(k==='1'||previous?.text==='1')));
   ...
   if(next.wait){digits.current={...next,location:`${at.bar}:${at.event}:${at.string}`,document:draftRef.current,undoDepth:depth};scheduleDigitExpiry();}
   return;
  }
  ...
  if(k.toLowerCase()==='x'){e.preventDefault();cancelDigits();setDraft(d=>patchEvent(enterFretWithDuration(d,active,0,entryDuration),barIndex,eventIndex,{dead:true}));advance();return;}
 ...
 const sheet=<div className="etudeEditorPreview">{!mobile&&<div className="etudePaperHeading" role="button" tabIndex="0" onClick={()=>showProperties('info')} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();showProperties('info');}}}><span>FRETIVA LAB</span><h3>{draft.english||draft.title}</h3><p>{draft.bpm} BPM · {draft.meter.join('/')} · Guitar Study</p></div>}{result.score?<EditorScore score={result.score} mobile={mobile} cursor={active} onSelect={select} onKeyDown={keyDown} onMove={move} onMessage={setMessage} zoom={zoom} onZoomChange={setZoom} zoomController={zoomController} view={view} tabRhythm={tabRhythm} playPosition={playPosition} allowDrag={!mobile||mobileDrag}/>:<p>잘못된 입력을 속성에서 수정하세요. 입력 데이터는 그대로 보존되어 있습니다.</p>}</div>;
 ...
 if(mobile)return <dialog ref={dialog} className="etudeEditor etudeEditor--mobile mobileScoreWorkspace" aria-label="악보 편집" onKeyDown={dialogKeyDown} onCancel={e=>{e.preventDefault();mobileSheet?setMobileSheet(null):close();}}>
  <header className="mobileScoreHeader"><button type="button" aria-label="악보 편집 뒤로" onClick={close}>←</button><strong>악보 편집</strong>{historyTools}<button type="button" aria-label="악보 저장" className="etudeEditorSave" onClick={()=>save()}>저장</button></header>
  {sheet}
  <MobileScoreInput cursor={active} event={event} meter={draft.meter} duration={entryDuration} onDuration={setDuration} onKey={mobileKey} onBar={delta=>setBar(Math.max(0,Math.min(draft.measures.length-1,barIndex+delta)))} onAddBar={addEmpty} onPick={directPick} onBatch={pattern=>setDraft(d=>applyPicking(d,{pattern,skipLegato:true}))} onDetails={section=>{setMobileSheet(section);if(section!=='menu')setPropertySection(section);}} twoDigit={twoDigit} onDigitMode={toggleDigitMode}
   playback={<ScorePlayback score={result.score} disabled={Boolean(result.errors.length||result.issues?.length)} compact controller={playbackController} startAt={active} onPosition={setPlayPosition} onBpm={bpm=>setDraft(d=>({...d,bpm}))}/>}/>
  {mobileSheet&&<section className="mobileEditorSheet" aria-label="악보 상세 설정"><header><strong>{mobileSheet==='menu'?'악보 메뉴':mobileSheet==='note'?'음표 속성':mobileSheet==='bar'?'마디 편집':'곡 정보'}</strong><button type="button" aria-label="상세 설정 닫기" onClick={()=>setMobileSheet(null)}>닫기 ×</button></header><div className="mobileSheetContent"><div className="mobileDetailTabs">{[['menu','메뉴'],['note','음표'],['bar','마디'],['info','곡 정보']].map(([section,label])=><button type="button" key={section} aria-pressed={mobileSheet===section} onClick={()=>{setMobileSheet(section);if(section!=='menu')setPropertySection(section);}}>{label}</button>)}</div>{mobileSheet==='menu'?menus:controls}{mobileSheet==='bar'&&beatTools}{mobileSheet==='note'&&techniqueTools}{mobileSheet==='menu'&&<details><summary>입력 설정</summary>{autoAdvanceOption}<label className="etudeEditorCheck"><input type="checkbox" checked={mobileDrag} onChange={e=>setMobileDrag(e.target.checked)}/>음표 드래그 이동</label><p>10·12·15는 1 다음 숫자를 420ms 안에 누릅니다. 20–24는 두 자리 모드를 켜세요. 숫자는 현재 위치를 수정합니다. 다음 위치는 ← →로 이동하세요.</p></details>}{mobileSheet==='menu'&&<button type="button" onClick={()=>setResetOpen(v=>!v)}>악보 초기화</button>}{batchPanels}{original&&<button type="button" onClick={()=>{setDraft({...toScoreDocument(original),id:draft.id,kind:'user'});setBar(0);}}>기본 악보 복원</button>}<p>간단 악보는 빠른 연습용입니다. 정밀한 악보 편집은 전문 악보 프로그램을 이용해 주세요.</p></div></section>}
  <input ref={file} type="file" accept={SCORE_FILE_ACCEPT} aria-label="악보 파일 선택" hidden onChange={importFile}/>
  {message&&<div className="mobileEditorNotice" role="status" onClick={()=>setMessage('')}>{message}</div>}
  {(result.errors.length>0||result.issues?.length>0)&&<button type="button" className="mobileValidation" onClick={()=>{setMobileSheet('note');setPropertySection('note');setMessage([...result.errors,...result.issues??[]].slice(0,3).join(' / '));}}>입력 확인 · 초안 저장 가능</button>}
 ...
 return <dialog ref={dialog} className={`etudeEditor etudeEditor--${mobile?'mobile':'desktop'}`} aria-label="악보 편집" onKeyDown={dialogKeyDown} onCancel={e=>{e.preventDefault();close();}}>
