...
export default function ScoreEditor({score,document:initialDocument,original,mobile,onClose,onSave,onImportPdf,compactImport=false}) {
 ...
 return <button type="button" key={kind} aria-label={label} title={label} disabled={(!quickSelecting||quick)&&(event.rest||!event.notes.length)} aria-pressed={quick?(base==='slur'&&slurStart===event.id):quickSelecting?quickItems.some(a=>a[0]===kind):selected} onClick={()=>{if(!quick&&quickSelecting){if(!quickItems.some(a=>a[0]===kind))updateQuick([...quickItems,[kind,label,shortLabel]]);setQuickVisible(true);return;}connection(kind);}}>{literal?<span className="techniqueShort">{literal}</span>:<EditorMusicIcon kind={base}/>} {caption&&<span className="techniqueCaption">{caption}</span>} {quick&&base.startsWith('bend-')&&<span>{Number(amount??2)===2?'Full':Number(amount)===1?'½':'¼'}</span>}</button>;
 };
