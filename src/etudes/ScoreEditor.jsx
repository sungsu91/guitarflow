...
export default function ScoreEditor({score,document:initialDocument,original,mobile,onClose,onSave,onImportPdf,compactImport=false}) {
 ...
 const select=useCallback((c,{seekPlayback=true}={})=>{if(isPiano(draftRef.current)){const selected=draftRef.current.measures[c.bar]?.events[c.event];const hand=selected?.voice??c.hand??inputHand;c={...c,hand,eventId:selected?.id,onset:selected?.onset};pianoPositions.current[hand]=pianoPosition(draftRef.current,c);setInputHand(hand);}if(isFretted(draftRef.current.instrument)&&c.string!==cursorRef.current.string||!draftRef.current.measures[c.bar]?.events[c.event]?.notes.some(n=>n.id===c.noteId))c={...c,noteId:undefined};c={...c,string:isFretted(draftRef.current.instrument)?Math.min(c.string,draftRef.current.tuning.length):1,mode:isFretted(draftRef.current.instrument)?c.mode:'staff'};cancelDigits();cursorRef.current=c;setCursor(c);setCandidates([]);setPropertySection(c.target==='bar'?'bar':'note');const selected=draftRef.current.measures[c.bar]?.events[c.event];if(selected&&(!selected.blank||selected.tuplet)&&dottedMode==='off'&&!tripletInput)setSelectedDuration(c.inputDuration??selected.duration);if(seekPlayback)playbackController.current?.seek(c);if(c.midi!==undefined)setPitch(c.midi);},[cancelDigits,dottedMode,tripletInput,inputHand]);
 ...
 const techniqueTools=!fretted?<div>{draft.instrument!=='drums'&&techniqueButton('tie','붙임줄')}{techniqueButton('clear','연결 지우기')}</div>:<><div className="techniqueQuickActions"><button type="button" aria-pressed={quickSelecting} onClick={()=>{setQuickSelecting(v=>!v);setQuickVisible(true);}}>퀵 등록</button><button type="button" onClick={()=>{setQuickVisible(true);setQuickOpen(true);setQuickSelecting(false);setMobileSheet(null);setToolSection(null);}}>퀵 패널</button></div><div className="techniqueGroup" role="group" aria-label="음 연결">{[['H','해머온'],['P','풀오프'],['S','슬라이드'],['tie','붙임줄'],['slur','이음줄(슬러)','이음줄']].map(args=>techniqueButton(...args))}</div><div className="techniqueGroup techniqueExpressionGroup" role="group" aria-label="음 표현">{[['palmMute','P.M. 팜 뮤트','팜 뮤트'],['vibrato','비브라토'],['harmonic','하모닉스'],['arpeggio-up','아르페지오 상행','상행'],['arpeggio-down','아르페지오 하행','하행']].map(args=>techniqueButton(...args))}</div><div className="techniqueGroup" role="group" aria-label="열린 연결과 괄호">{[['let-ring','열린 붙임줄'],['slide-in-up','슬라이드 인 상행'],['slide-in-down','슬라이드 인 하행'],['slide-out-up','슬라이드 아웃 상행','아웃 ↗'],['slide-out-down','슬라이드 아웃 하행','아웃 ↘'],['parentheses','괄호 음표','(음)']].map(args=>techniqueButton(...args))}</div><div className="bendAmountOptions" role="radiogroup" aria-label="벤딩 음정">{[[2,'Full'],[1,'½'],[.5,'¼']].map(([value,label])=><button type="button" key={value} role="radio" aria-checked={bendAmount===value} onClick={()=>setBendAmount(value)}>{label}</button>)}</div><div className="techniqueGroup" role="group" aria-label="벤드와 릴리스">{[['bend-prebend','프리벤드','프리벤드'],['bend-up','벤드 올림','올림'],['bend-hold','벤드 유지','유지'],['bend-release','벤드 릴리스','내림'],['bend-up-release','벤드 올렸다 내림','올림·내림']].map(([kind,label,short])=>techniqueButton(kind+':'+bendAmount,label,short))}</div><div className="techniqueFooter"><p className="techniqueApplyHint" role="status">{event.rest?'주법을 적용할 음을 선택하세요.':techniqueHint}</p>{techniqueButton('clear','주법 지우기','지우기')}</div></>;
 const beatTools=<>
 ...
 const pitchInput=draft.instrument==='drums'?<DrumInput onTechnique={value=>{setDraft(d=>setDrumTechnique(d,active,value));focusScore();}} onRimshot={()=>{enterPitches([38],inputHand,false,'rimshot');void auditionDrum(38,'rimshot');focusScore();}} onRest={()=>{try{setDraft(d=>insertDrumLowerRest(d,active));focusScore();}catch(error){setMessage(error.message);}}} hat={drumHat} onHat={setDrumHat} batch={drumBatch} onBatch={value=>{setDrumBatch(value);focusScore();}} event={event} cursor={active} meter={draft.meter} preview={drumPreview} onPreview={setDrumPreview} volume={drumVolume} onVolume={setDrumVolume} onKey={mobileKey} onEnter={pitches=>{pitches.forEach(midi=>void auditionDrum(midi));enterPitches(pitches,inputHand,false,null);focusScore();}}/>:<KeyboardInput hand={inputHand} onInputHand={switchHand} chordMode={chordMode} onChordMode={toggleChord} instrument={draft.instrument} event={event} cursor={active} desktop={!mobile} onEnter={(pitches,hand,options)=>{playbackController.current?.stop();enterPitches(pitches,hand);if(options?.focus!==false)focusScore();}} onKey={mobileKey} onSelect={n=>select({...active,noteId:n.id,midi:n.midi,mode:'staff'})} onHand={hand=>{try{const next=movePianoHand(draftRef.current,active,hand);setDraft(next.document);select({...next.cursor,editing:true});}catch(error){setMessage(error.message);}}}/>;
