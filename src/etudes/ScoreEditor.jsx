...
import useScorePreview from './useScorePreview.js';
...
import {setNoteConnection,setEventDuration,resolveFretInput,deleteTone,moveFingering,setRest,insertEvent,moveTone,splitEvent,applyPicking,nextEntry,cursorStep,copyBars,pasteBars} from './editorCommands.js';
...
function Controls({draft,setDraft,bar,setBar,event,setEvent,section='note',mobile}) {
 ...
 return <div className="etudeEditorControls">
  ...
  <div className="etudeEditorEvent">
   ...
   <label className="etudeEditorCheck"><input type="checkbox" checked={n.rest&&!n.blank} onChange={e=>edit(d=>{const target=d.measures[bar].events[event];target.rest=e.target.checked;target.blank=false;target.technique=null;if(!target.rest&&!target.notes.length){const grip=d.measures[bar].chord;const string=grip?6-grip.frets.findIndex(f=>f!==null):1;target.notes=[{string,fret:grip?grip.frets[6-string]:0}];}})}/>쉼표</label>
   {!n.rest&&<>
...
export default function ScoreEditor({score,document:initialDocument,original,mobile,onClose,onSave,onImportPdf,compactImport=false}) {
 ...
 const draftRef=useRef(draft),undoStack=useRef([]),redoStack=useRef([]),digits=useRef(null),clipboard=useRef([]),savedRef=useRef(draft),openedRef=useRef(draft);
 ...
 const [instrument]=useScoreInstrument();
 const {play:preview,stop:stopPreview}=useScorePreview(instrument,setMessage);
 ...
 const select=useCallback(c=>{setBeamSelection(null);cancelDigits();cursorRef.current=c;setCursor(c);setCandidates([]);setPropertySection(c.target==='bar'?'bar':'note');const selected=draftRef.current.measures[c.bar]?.events[c.event];if(selected&&(!selected.blank||selected.tuplet)&&dottedMode==='off'&&!tripletInput)setSelectedDuration(selected.duration);playbackController.current?.seek(c);if(c.midi!==undefined)setPitch(c.midi);},[cancelDigits,dottedMode,tripletInput]);
 useLayoutEffect(()=>{const node=dialog.current,overflow=document.body.style.overflow;document.body.style.overflow='hidden';node.showModal();if(!mobile)node.querySelector('[data-score-input]')?.focus({preventScroll:true});return()=>{node.close();document.body.style.overflow=overflow;};},[]);
 ...
 const keyDown=(e,tabInput=false)=>{
  ...
  if(/^\d$/.test(k)){
   ...
   void preview(draftRef.current,at);
   return;
  }
  if(k==='ArrowLeft'||k==='ArrowRight'){e.preventDefault();select(cursorStep(draft,active,k==='ArrowLeft'?-1:1));return;}
  if(k==='ArrowUp'||k==='ArrowDown'){e.preventDefault();if(cursor.mode==='staff'&&!tabInput){const midi=(cursor.midi??pitch)+(k==='ArrowUp'?1:-1);setPitch(midi);setCursor(c=>({...c,midi}));}else select({...active,mode:tabInput?'tab':active.mode,string:Math.max(1,Math.min(6,cursor.string+(k==='ArrowUp'?-1:1)))});return;}
  ...
  if(k.toLowerCase()==='x'){e.preventDefault();cancelDigits();try{commitRhythm(active,'mute');void preview(draftRef.current,active);}catch(err){setMessage(err.message);}return;}
  if(k==='Backspace'||k==='Delete'){e.preventDefault();stopPreview();cancelDigits();setDraft(d=>deleteTone(d,active));return;}
  if(k.toLowerCase()==='r'){e.preventDefault();stopPreview();cancelDigits();try{commitRhythm(active,'rest');advance();}catch(err){setMessage(err.message);}return;}
 ...
 const connection=kind=>{setTechniqueHint(({H:'같은 줄의 연속된 단음을 낮은 프렛에서 높은 프렛으로 연결합니다.',P:'같은 줄의 연속된 단음을 높은 프렛에서 낮은 프렛으로 연결합니다.',S:'같은 줄의 연속된 두 단음을 다른 프렛으로 슬라이드합니다.',tie:'다음 같은 음높이로 소리를 이어줍니다.',vibrato:'선택한 음에 비브라토를 적용합니다.',harmonic:'선택한 줄의 자연 하모닉스 위치에 적용합니다.','arpeggio-up':'현재 화음을 낮은 줄에서 높은 줄 순서로 연주합니다.','arpeggio-down':'현재 화음을 높은 줄에서 낮은 줄 순서로 연주합니다.',clear:'선택한 음의 주법 표시를 지웁니다.'})[kind]);try{setDraft(d=>setNoteConnection(d,active,kind));}catch(e){setMessage(e.message);}};
 const techniqueTools=<>{[['H','해머온'],['P','풀오프'],['S','슬라이드'],['tie','붙임줄'],['vibrato','비브라토'],['harmonic','하모닉스'],['arpeggio-up','아르페지오 상행'],['arpeggio-down','아르페지오 하행'],['clear','주법 지우기']].map(([kind,label])=><button type="button" key={kind} aria-label={label} disabled={event.rest||!event.notes.length} aria-pressed={['H','P','S'].includes(kind)?event.technique===kind:kind==='tie'?Boolean(event.tieTo):kind==='vibrato'?Boolean(event.vibrato):kind==='harmonic'?Boolean(event.notes.find(n=>n.string===active.string)?.harmonic):kind.startsWith('arpeggio-')?event.arpeggio===kind.slice(9):undefined} onClick={()=>connection(kind)}><EditorMusicIcon kind={kind}/><span>{label}</span></button>)}<p className="techniqueApplyHint" role="status">{event.rest?'주법을 적용할 음을 선택하세요.':techniqueHint}</p></>;
 const beatTools=<>
 ...
 const notationViews=<div className={mobile?'mobileNotationViews':'desktopNotationViews'} role="group" aria-label="보표 보기">{[['both','오선보+TAB'],['staff','오선보'],['tab','TAB']].map(([value,label])=><button type="button" key={value} aria-pressed={view===value} onClick={()=>{setView(value);if(value!=='both')select({...active,mode:value==='tab'?'tab':'staff'});}}>{label}</button>)}</div>;
 ...
 const controls=<div className="etudePropertiesPanel"><h3>{propertySection==='note'?'음표 속성':propertySection==='bar'?'마디 속성':'곡 정보'}</h3>{!mobile&&propertySection==='note'&&<EditorInputPanel cursor={active} event={event} onString={string=>{select({...active,string,mode:'tab',target:undefined});focusScore();}} onFret={fret=>{try{commitRhythm(active,'note',fret);void preview(draftRef.current,active);}catch(error){setMessage(error.message);}}} onDelete={()=>{stopPreview();setDraft(d=>deleteTone(d,active));}}/>}<Controls {...{draft,setDraft,bar:barIndex,setBar,event:eventIndex,setEvent,mobile}} section={propertySection}/>{tools}</div>;
 ...
 const sheet=<div className="etudeEditorPreview">{mobile&&<div className="mobileNotationTools">{notationViews}<div className="mobileDurationRow"><DurationButtons value={selectedDuration} onChange={setDuration} compact/><button type="button" aria-label="쉼" aria-pressed={restMode} onClick={()=>mobileKey('r')}>쉼</button>{modifiers}</div>{(dottedMode!=='off'||tupletMode!=='off')&&<output className="rhythmInputStatus" aria-live="polite">{inputStatus}</output>}</div>}{!mobile&&<div className="etudePaperHeading" role="button" tabIndex="0" onClick={()=>showProperties('info')} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();showProperties('info');}}}><span>FRETIVA LAB</span><h3>{draft.english||draft.title}</h3><p>{draft.bpm} BPM · {draft.meter.join('/')} · Guitar Study</p></div>}{result.score?<EditorScore score={result.score} mobile={mobile} cursor={active} beamSelection={beamSelection} beamRangeMode={beamRangeMode} onBeamRange={toggleBeamRange} onLayoutChange={settings=>setDraft(d=>({...d,viewSettings:{...d.viewSettings,...settings}}))} onSelect={selectInScore} onKeyDown={keyDown} onMove={move} onMessage={setMessage} zoom={zoom} onZoomChange={setZoom} zoomController={zoomController} view={view} tabRhythm={tabRhythm} playPosition={playPosition} allowDrag={!beamRangeMode&&(!mobile||mobileDrag)}/>:<p>잘못된 입력을 속성에서 수정하세요. 입력 데이터는 그대로 보존되어 있습니다.</p>}{!mobile&&rangeTools}</div>;
 const mobileKey=key=>keyDown({key,target:dialog.current,preventDefault(){}},true);
 const directPick=value=>{cancelDigits();if(event.rest){setMessage('피킹은 음표가 있는 위치에서 입력하세요.');return;}setDraft(d=>patchEvent(d,barIndex,eventIndex,{pickStroke:value}));void preview(draftRef.current,active);if(value)select(cursorStep(draftRef.current,active,1));};
 ...
 if(mobile)return <dialog ref={dialog} className="etudeEditor etudeEditor--mobile mobileScoreWorkspace editorDesign" aria-label="악보 편집" onKeyDown={dialogKeyDown} onCancel={e=>{e.preventDefault();mobileSheet?closeMobileTool():close();}}>
  ...
  {sheet}
  <MobileScoreInput openTool={mobileSheet} onTool={toggleMobileTool} onCloseTool={closeMobileTool} techniques={mobileTechniques} beamSelection={beamSelection} onBeamApply={applyBeamRange} feedback={mobileFeedback} beamRangeMode={beamRangeMode} onBeamRange={()=>{setBeamRangeMode(v=>!v);setBeamSelection(null);}} cursor={active} event={event} meter={draft.meter} onKey={mobileKey} onBar={delta=>setBar(Math.max(0,Math.min(draft.measures.length-1,barIndex+delta)))} onAddBar={addEmpty} onPick={directPick} onBatch={pattern=>setDraft(d=>applyPicking(d,{pattern,skipLegato:true}))}
   playback={<ScorePlayback onBeforePlay={stopPreview} score={result.score} disabled={Boolean(result.errors.length||result.issues?.length)} dock compact controller={playbackController} startAt={active} onPosition={setPlayPosition} onBpm={bpm=>setDraft(d=>({...d,bpm}))}/>}/>
