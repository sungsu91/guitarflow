...
export default function ScoreEditor({score,document:initialDocument,original,mobile,onClose,onSave,onImportPdf,compactImport=false}) {
 ...
 const commitSave=document=>{
  ...
  const close=saveRequest.close;setSaveRequest(null);if(close)finishClose();
 };
 ...
 const connection=kind=>{setTechniqueHint(({H:'같은 줄의 다음 음: 낮은 → 높은 프렛',P:'같은 줄의 다음 음: 높은 → 낮은 프렛',S:'같은 줄의 다음 음: 다른 프렛으로 연결',tie:'다음 같은 음높이로 소리 연결',vibrato:'선택한 음에 비브라토 적용',harmonic:'선택한 줄의 자연 하모닉스 위치에 적용','arpeggio-up':'현재 화음: 낮은 줄 → 높은 줄','arpeggio-down':'현재 화음: 높은 줄 → 낮은 줄',clear:'선택한 음의 주법 지우기'})[kind]);try{setDraft(d=>setNoteConnection(d,active,kind));}catch(e){setMessage(e.message);}};
 ...
 const techniqueTools=<><div className="techniqueGroup" role="group" aria-label="음 연결">{[['H','해머온'],['P','풀오프'],['S','슬라이드'],['tie','붙임줄']].map(args=>techniqueButton(...args))}</div><div className="techniqueGroup" role="group" aria-label="음 표현">{[['vibrato','비브라토'],['harmonic','하모닉스'],['arpeggio-up','아르페지오 상행','상행'],['arpeggio-down','아르페지오 하행','하행']].map(args=>techniqueButton(...args))}</div><div className="techniqueFooter"><p className="techniqueApplyHint" role="status">{event.rest?'주법을 적용할 음을 선택하세요.':techniqueHint}</p>{techniqueButton('clear','주법 지우기','지우기')}</div></>;
 const beatTools=<>
 ...
 const sheet=<div className="etudeEditorPreview">{mobile&&<div className="mobileNotationTools">{notationViews}<div className="mobileDurationRow"><DurationButtons value={selectedDuration} onChange={setDuration} compact/><button type="button" aria-label="쉼" aria-pressed={restMode} onClick={()=>mobileKey('r')}>쉼</button>{modifiers}</div>{(dottedMode!=='off'||tupletMode!=='off')&&<output className="rhythmInputStatus" aria-live="polite">{inputStatus}</output>}</div>}{!mobile&&<div className="etudePaperHeading" role="button" tabIndex="0" onClick={()=>showProperties('info')} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();showProperties('info');}}}><span>FRETIVA LAB</span><h3>{draft.title}</h3><p>{draft.bpm} BPM · {draft.meter.join('/')} · Guitar Study</p></div>}{result.score?<EditorScore score={result.score} mobile={mobile} cursor={active} beamSelection={beamSelection} beamRangeMode={beamRangeMode} onBeamRange={toggleBeamRange} onLayoutChange={settings=>setDraft(d=>({...d,viewSettings:{...d.viewSettings,...settings}}))} onSelect={selectInScore} onKeyDown={keyDown} onMove={move} onMessage={setMessage} zoom={zoom} onZoomChange={setZoom} zoomController={zoomController} view={view} tabRhythm={tabRhythm} playPosition={playPosition} allowDrag={!beamRangeMode&&(!mobile||mobileDrag)}/>:<p>잘못된 입력을 속성에서 수정하세요. 입력 데이터는 그대로 보존되어 있습니다.</p>}{!mobile&&rangeTools}</div>;
 ...
 const removeBar=()=>{try{stopPreview();playbackController.current?.stop();setDraft(d=>deleteMeasure(d,barIndex));setBar(Math.min(barIndex,draft.measures.length-2));setMessage(`${barIndex+1}마디를 삭제했습니다. 실행 취소로 복구할 수 있습니다.`);}catch(e){setMessage(e.message);}};
