...
export default function ScoreEditor({score,document:initialDocument,original,mobile,onClose,onSave,onImportPdf,compactImport=false}) {
 ...
 const pitchInput=draft.instrument==='drums'?<DrumInput onRest={()=>{try{setDraft(d=>insertDrumLowerRest(d,active));focusScore();}catch(error){setMessage(error.message);}}} hat={drumHat} onHat={setDrumHat} batch={drumBatch} onBatch={value=>{setDrumBatch(value);focusScore();}} event={event} cursor={active} meter={draft.meter} preview={drumPreview} onPreview={setDrumPreview} volume={drumVolume} onVolume={setDrumVolume} onKey={mobileKey} onEnter={pitches=>{pitches.forEach(midi=>void auditionDrum(midi));enterPitches(pitches);focusScore();}}/>:<KeyboardInput hand={inputHand} onInputHand={switchHand} chordMode={chordMode} onChordMode={toggleChord} instrument={draft.instrument} event={event} cursor={active} desktop={!mobile} onEnter={(pitches,hand,options)=>{playbackController.current?.stop();enterPitches(pitches,hand);if(options?.focus!==false)focusScore();}} onKey={mobileKey} onSelect={n=>select({...active,noteId:n.id,midi:n.midi,mode:'staff'})} onHand={hand=>{try{const next=movePianoHand(draftRef.current,active,hand);setDraft(next.document);select({...next.cursor,editing:true});}catch(error){setMessage(error.message);}}}/>;
