...
export default function PracticeSheet({model,mobile,heading,title,lessonTips,footer}) {
 ...
 {!mobile&&!focus&&<div className="etudeInlineBarCount etudeDesktopBarCount" role="group" aria-label="한 줄 마디 수"><span>보기</span>{[0,1,2,3,4].map(n=><button key={n} type="button" aria-label={n?`한 줄 ${n}마디`:'한 줄 마디 수 자동'} aria-pressed={measuresPerRow===n} onClick={()=>model.setMeasuresPerRow(n)}>{n||'자동'}</button>)}</div>}
