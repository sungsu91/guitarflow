...
function silentParts(start,end,voice){const result=[];while(start<end-1e-6){const duration=[1,2,4,8,16,32,64].find(d=>1920/d<=end-start+1e-6);if(!duration)throw Error(ko["etudes.selectTheEntireTripletGroup"]);const e=blankEvent(start,String(duration));if(voice)e.voice=voice;result.push(e);start+=ticksOf(e);}return result;}
export function copyScoreRange(d,range){
 ...
 const a=items.find(x=>x.bar===range.start.bar&&x.index===range.start.event),b=items.find(x=>x.bar===range.end.bar&&x.index===range.end.event);if(!a||!b)throw Error(ko["etudes.reselectTheNoteRangeToCopy"]);
...
export function pasteScoreRange(d,cursor,clip){
 if(!clip?.events?.length)throw Error(ko["etudes.copyAScoreRangeWithCtrlCFirst"]);
 if(clip.instrument!==(d.instrument??'guitar')||JSON.stringify(clip.tuning)!==JSON.stringify(d.tuning))throw Error(ko["etudes.pasteIntoAScoreWithTheSameInstrumentAndTuningAsThe"]);
 let rows=timeline(d);const target=d.measures[cursor.bar]?.events[cursor.event];if(!target)throw Error(ko["etudes.selectAPastePosition"]);const start=rows[cursor.bar].start+target.onset,end=start+clip.span;
 const result=structuredClone(d);while(rows.at(-1).end<end){if(result.measures.length>=64)throw Error(ko["etudes.theMaximumIs64Bars"]);const bar=blankMeasure(rows.at(-1).meter);if(target.voice){bar.events= ['right','left'].flatMap(voice=>bar.events.map(e=>({...structuredClone(e),id:newId('event'),voice})));}result.measures.push(bar);rows=timeline(result);}
 for(const row of rows)for(const e of result.measures[row.bar].events){const at=row.start+e.onset,stop=at+durationOf(e);if(!e.blank&&at<end&&stop>start&&(at<start||stop>end))throw Error(ko["etudes.thePasteBoundaryFallsInsideAnExistingNoteAlignItWithA"]);}
 // Validate before replacing: never silently cut an existing sounding note.
 for(const row of rows){const events=result.measures[row.bar].events;result.measures[row.bar].events=events.flatMap(e=>{const at=row.start+e.onset,stop=at+ticksOf(e);if(at>=end||stop<=start)return [e];if((at<start||stop>end)&&!e.blank)throw Error(ko["etudes.thePasteBoundaryFallsInsideAnExistingNoteAlignItWithA"]);return [...(at<start?silentParts(e.onset,start-row.start,e.voice):[]),...(stop>end?silentParts(end-row.start,stop-row.start,e.voice):[])];});}
 const ids=new Map(clip.events.map(x=>[x.event.id,newId('event')])),groups=new Map();
 for(const item of clip.events){const e=structuredClone(item.event),at=start+item.at,stop=at+ticksOf(e),row=rows.find(r=>at>=r.start&&at<r.end);if(!row)throw Error(ko["etudes.checkThePasteRange"]);
 ...
 if(stop>row.end+1e-6){if(!e.blank)throw Error(ko["etudes.aCopiedNoteCrossesABarlineAdjustTheStartingBeatForThe"]);for(const r of rows.filter(r=>r.start<stop&&r.end>at))result.measures[r.bar].events.push(...silentParts(Math.max(at,r.start)-r.start,Math.min(stop,r.end)-r.start,e.voice));}else{e.onset=at-row.start;result.measures[row.bar].events.push(e);}
 }
