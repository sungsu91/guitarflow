...
export function chordDiagramErrors(shape,count,capacity,maximum=24){
 if(!shape)return [];
 const errors=[];
 if(shape.ranges&&(!Array.isArray(shape.ranges)||!shape.ranges.length||shape.ranges.some((r,i)=>!r||!Number.isFinite(r.startTick)||!Number.isFinite(r.endTick)||r.startTick<0||r.endTick>capacity||r.startTick>=r.endTick||(i>0&&r.startTick<shape.ranges[i-1].endTick))))errors.push('코드표 적용 박을 확인하세요.');
 if(shape.range&&(!Number.isFinite(shape.range.startTick)||!Number.isFinite(shape.range.endTick)||shape.range.startTick<0||shape.range.endTick>capacity||shape.range.startTick>=shape.range.endTick))errors.push('코드표 적용 시작·끝 위치를 확인하세요.');
 if(shape.fretWindow){const {start,end}=shape.fretWindow;if(!Number.isInteger(start)||!Number.isInteger(end)||start<1||end>maximum||start>end||shape.frets?.some(f=>f>0&&(f<start||f>end)))errors.push('코드표 표시 프렛 범위에 모든 운지가 포함되어야 합니다.');}
 if(shape.blankStrings&&(!Array.isArray(shape.blankStrings)||shape.blankStrings.some(s=>!Number.isInteger(s)||s<1||s>count||shape.frets[count-s]!==null)))errors.push('코드표의 빈 줄 표시를 확인하세요.');
 return errors;
}
...
export function attachChordDiagram(document,bar,shape,{autoFill=false}={}){
 ...
 if(shape&&(!shape.name.trim()||shape.name.length>40||shape.frets.length!==document.tuning.length||!shape.frets.some(f=>Number.isInteger(f))))errors.push('코드명과 하나 이상의 운지를 입력하세요.');
 ...
 if(ranges.some(({startTick,endTick})=>events.some(e=>(e.onset<startTick&&e.onset+duration(e)>startTick)||(e.onset<endTick&&e.onset+duration(e)>endTick))))throw Error('운지 자동 기입은 음표 경계에 맞춰 구간을 선택하세요.');
