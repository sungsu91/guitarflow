...
export default function useEtudeMetronome(bpm, { beatsPerBar = 4, beatUnit = 4, audible = true, downbeatAt, liveTempo = false } = {}) {
  ...
  const start = useCallback(async ({beatOffset=0,durationSeconds=Infinity}={}) => {
    stop();
    ...
    try {
      ...
      const s = { context, gain, origin, stepSeconds, positionOffset:beatOffset, firstBeat, oscillators: new Set(), cursor: createAudioTransportCursor({ originTime: clickOrigin, stepSeconds }), frame: 0, timer: 0 };
      session.current = s;
      const schedule = () => {
        ...
        batch.steps.filter(step=>step.time<origin+durationSeconds-1e-7).forEach(step => {
          ...
          const downbeat = downbeatRef.current ? downbeatRef.current(step.index+s.firstBeat) : (step.index+s.firstBeat) % beatsPerBar === 0;
  ...
  useEffect(() => {
    ...
    s.oscillators.forEach(o=>{try{o.stop();}catch{}});
    s.origin=now;s.positionOffset=position;s.stepSeconds=stepSeconds;s.firstBeat=Math.ceil(position);
    s.cursor=createAudioTransportCursor({originTime:now+(s.firstBeat-position)*stepSeconds,stepSeconds});
  },[bpm,beatsPerBar,beatUnit,liveTempo,stop]);
  useEffect(()=>stop,[stop]);
  useEffect(() => {
