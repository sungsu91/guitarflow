...
export default function usePracticeLayout(){
 const read=()=>{const v=window.visualViewport,width=v?.width??innerWidth,height=v?.height??innerHeight;return {width,height,left:v?.offsetLeft??0,top:v?.offsetTop??0,landscape:matchMedia('(orientation: landscape)').matches&&width>height&&Math.min(width,height)<900&&(matchMedia('(pointer: coarse)').matches||navigator.maxTouchPoints>0)};};
