...
export default function MobileScoreInput({desktop=false,openTool,onTool,onCloseTool,techniques,beamSelection,onBeamApply,repeatBar,repeatIssue,onRepeat,onNavigation,onDeleteBar,canDeleteBar,cursor,event,meter,beamRangeMode,onBeamRange,tabRhythm=true,tabBeamPosition='below',tabPickingPosition='below',onTabBeam,onPickingPosition,onKey,onBar,onCopyLine,onDeleteLine,onAddBar,onPick,onBatch,feedback,playback}) {
 ...
 const toolDock=<div className={`mobileToolDock${hasPanel?' has-connected-panel':''}`}>
  ...
  <div className="mobilePickRow"><span>직접</span>{[['down','Π'],['up','V'],[null,'지움']].map(([value,label])=><button type="button" key={label} aria-label={`직접 피킹 ${value==='down'?'다운':value==='up'?'업':'지움'}`} aria-pressed={Boolean(value&&event.pickStroke===value)} onClick={()=>onPick(value)}>{value?<EditorMusicIcon kind={value}/>:<X size={22}/>}</button>)}</div></div>:<div className="mobileBeamPopoverBody"><div className="tabDisplayOptions" role="group" aria-label="TAB 빔 표시"><span>TAB</span>{[['below','아래'],['above','위'],['hidden','빔 삭제']].map(([value,label])=><button type="button" key={value} aria-label={value==='hidden'?'TAB 빔 삭제':`TAB 빔 ${label} 표시`} aria-pressed={value==='hidden'?!tabRhythm:tabRhythm&&tabBeamPosition===value} onClick={()=>onTabBeam(value)}>{label}</button>)}</div>
