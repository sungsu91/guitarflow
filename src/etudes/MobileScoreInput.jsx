...
export default function MobileScoreInput({drums=false,fretted=true,pitchInput,desktop=false,openTool,onTool,onCloseTool,techniques,beamSelection,onBeamApply,repeatBar,repeatIssue,onRepeat,onNavigation,onResetBar,onDeleteBar,canDeleteBar,cursor,event,meter,beamRangeMode,onBeamRange,tabRhythm=true,tabBeamPosition='below',tabPickingPosition='below',onTabBeam,onPickingPosition,onKey,onBar,onCopyLine,onDeleteLine,onAddBar,onPick,onBatch,feedback,playback}) {
 ...
 const toolDock=<div className={`mobileToolDock${hasPanel?' has-connected-panel':''}`}>
  ...
  <div className="mobilePickRow"><span>직접</span>{[['down','Π'],['up','V'],[null,'지움']].map(([value,label])=><button type="button" key={label} aria-label={`직접 피킹 ${value==='down'?'다운':value==='up'?'업':'지움'}`} aria-pressed={Boolean(value&&event.pickStroke===value)} onClick={()=>onPick(value)}>{value?<EditorMusicIcon kind={value}/>:<X size={22}/>}</button>)}</div></div>:<div className="mobileBeamPopoverBody">{fretted&&<div className="tabDisplayOptions" role="group" aria-label="TAB 빔 표시"><span>TAB</span>{[['below','아래'],['above','위'],['hidden','빔 삭제']].map(([value,label])=><button type="button" key={value} aria-label={value==='hidden'?'TAB 빔 삭제':`TAB 빔 ${label} 표시`} aria-pressed={value==='hidden'?!tabRhythm:tabRhythm&&tabBeamPosition===value} onClick={()=>onTabBeam(value)}>{label}</button>)}</div>}
   ...
   <div className="mobileInputExtras"><button type="button" onClick={onAddBar}><Plus size={18}/><span>마디 추가</span></button><>{!drums&&<button type="button" className="is-delete" disabled={!canDeleteBar} onClick={onDeleteBar}><Trash2 size={18}/><span>마디 삭제</span></button>}</>{(fretted?[['picking','피킹'],['note','주법'],['beam','빔'],['repeat','반복']]:drums?[['beam','빔'],['repeat','반복']]:[['note','연결'],['beam','빔'],['repeat','반복']]).map(([kind,label])=><button type="button" key={kind} data-mobile-tool-toggle={kind} aria-label={`${label} 도구 ${openTool===kind?'닫기':'열기'}`} aria-expanded={openTool===kind} aria-pressed={openTool===kind} aria-controls={openTool===kind?`mobile-tool-${kind}`:undefined} onClick={()=>onTool(kind)}><span>{label}</span>{(desktop?openTool===kind:openTool!==kind)?<ChevronDown size={18}/>:<ChevronUp size={18}/>}</button>)}{drums&&<><button type="button" disabled={event.rest||!event.notes.length} title="현재 위치의 드럼 음 전체 삭제" onClick={onDeleteLine}>삭제</button><button type="button" aria-label="현재 마디 초기화" title="현재 마디의 음을 비웁니다 · 실행 취소 가능" onClick={onResetBar}>초기화</button></>}</div>
  </div>;
 return <section ref={panel} className={desktop?"desktopScoreInput":"mobileScoreInput"} aria-label="통합 악보 입력">
  {desktop&&!drums?<>{inputPads}{toolDock}</>:<>{toolDock}{inputPads}</>}
