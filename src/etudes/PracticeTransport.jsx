...
export default function PracticeTransport({fixedInstrument=null,bpm,onBpm,meter,beat,playing,paused,disabled,onStart,onStop,onPause,onResume,click,onClickSound,sound,onSound,instrument,onInstrument,error}){
 ...
 <div className="scorePracticeSound" role="group" aria-label="악보 소리"><button type="button" aria-pressed={sound} onClick={onSound}>악보 소리 {sound?'켜짐':'꺼짐'}</button><label>음색<select aria-label="악보 소리 음색" value={instrument} onChange={e=>onInstrument(e.target.value)}>{fixedInstrument?<option value={fixedInstrument}>{fixedInstrument==='drums'?'드럼':'피아노'}</option>:<><option value="clean-guitar">클린 기타</option><option value="piano">피아노</option></>}</select></label></div>{error&&<p role="alert">{error}</p>}</section>;
}
