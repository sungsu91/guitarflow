...
export function setNoteConnection(d,c,kind){
 ...
 if(kind==='parentheses'||kind.startsWith('bend-')){
  ...
  const [phase,rawAmount]=kind.slice(5).split(':');const amount=rawAmount===undefined?2:Number(rawAmount);if(![.5,1,2].includes(amount))throw Error('벤드 음정을 확인하세요.');if(kind!=='parentheses'&&!['up','hold','release','up-release','prebend'].includes(phase))throw Error('벤드 형태를 확인하세요.');
...
export function setEntryDuration(d,c,duration){
 ...
 if(event.tuplet)throw Error('3연음 묶음을 해제한 뒤 길이를 변경하세요.');
 ...
 if(following.some(e=>e.tuplet))throw Error('뒤의 셋잇단음표 묶음을 이동할 수 없습니다.');
 const end=last===c.event?event.onset+ticksOf({duration}):measure.events[last].onset+ticksOf(measure.events[last])+delta;
 if(end>capacity)throw Error('뒤의 음표를 유지하면 마디 길이를 넘습니다. 먼저 다음 마디로 옮기거나 빈 시간을 확보하세요.');
 ...
 if(onset!==capacity)throw Error('이 위치에서는 선택한 길이로 변경할 수 없습니다.');
