...
export default function EditorInputPanel({cursor,event,onString,onFret,onDelete}) {
 const tone=event.notes.find(n=>n.string===cursor.string);
 return <aside className="etudeContextPanel" aria-label="빠른 입력 패널">

  <div className="etudeCurrentNote" aria-live="polite"><span className="etudePanelEyebrow">현재 선택</span><div><DurationIcon value={event.duration}/><p><strong>{event.rest?'빈 박 · 쉼표':tone?`${cursor.string}번줄 ${tone.fret}프렛`:'선택 줄은 비어 있음'}</strong><small>{cursor.bar+1}마디 · {event.onset/480+1}박 · {cursor.string}번줄 · {event.duration==='1'?'온음표':`${event.duration}분음표`}</small></p></div></div>
  <>
   <div className="etudeInputField"><span className="etudePanelEyebrow">줄 선택</span><div className="etudeStringButtons">{[1,2,3,4,5,6].map(string=><button type="button" key={string} aria-label={`입력 ${string}번줄`} aria-pressed={cursor.string===string} onClick={()=>onString(string)}>{string}</button>)}</div></div>
   ...
   <button type="button" className="etudePanelDelete" onClick={onDelete} disabled={!tone}>선택 줄의 음 삭제</button>
  </>
 </aside>;
}
