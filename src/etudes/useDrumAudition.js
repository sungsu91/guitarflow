...
export default function useDrumAudition(enabled,volume,onError){
 ...
 return async midi=>{if(!current.current.enabled||!current.current.volume)return;try{const audio=await resumeSharedAudioContext();if(!alive.current||!audio||!current.current.enabled)return;await prepareDrumSamples(audio);if(!alive.current||!current.current.enabled)return;if(!output.current){output.current=audio.createGain();output.current.connect(getAudioBusInput(AUDIO_BUS_IDS.INSTRUMENT,audio));}output.current.gain.setValueAtTime(current.current.volume*.8,audio.currentTime);if([42,44,46].includes(midi))hat.current?.release(audio.currentTime+.003);const source=scheduleDrum(audio,{midi},audio.currentTime+.003,output.current,.46);if([42,44,46].includes(midi))hat.current=source;}catch(error){current.current.onError?.(error.message);}};
}
