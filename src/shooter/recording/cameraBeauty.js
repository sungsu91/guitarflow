...
// face tracking, geometry changes, or per-frame canvas allocation.
const processors = new WeakMap();
export const BEAUTY_LEVELS = ['끔', '자연', '부드럽게'];
...
uniform vec4 pulls[4]; uniform vec4 radii;
vec2 pull(vec2 p, vec4 point, float radius) {
  vec2 d=(p-point.xy)*vec2(pixel.y/pixel.x,1.0);
  float t=clamp(length(d)/max(radius,0.0001),0.0,1.0);
  float falloff=(1.0-t*t); falloff*=falloff;
  return point.zw*falloff*strength;
}
varying vec2 uv;
void main(){
  vec2 coord=uv+pull(uv,pulls[0],radii.x)+pull(uv,pulls[1],radii.y)
    +pull(uv,pulls[2],radii.z)+pull(uv,pulls[3],radii.w);
  vec3 c=texture2D(frame,coord).rgb;
  vec3 sum=c; float total=1.0;
  for(int x=-1;x<=1;x++) for(int y=-1;y<=1;y++) {
    vec3 n=texture2D(frame,coord+vec2(float(x),float(y))*pixel*2.0).rgb;
    vec3 delta=n-c;
    float weight=exp(-dot(delta,delta)*100.0);
  ...
  vec3 soft=mix(c,sum/total,skin*strength*0.65);
  soft+=skin*strength*0.055*soft*(1.0-soft);
...
function createProcessor() {
  ...
  const pullValues = new Float32Array(16), radiusValues = new Float32Array(4);
  const dispose = () => {
    tracker?.dispose();
  ...
  try {
    ...
    const pulls = gl.getUniformLocation(program, 'pulls[0]');
    const radii = gl.getUniformLocation(program, 'radii');
    ...
    return { dispose, render(video, level) {
      ...
      pullValues.fill(0); radiusValues.fill(0);
      shape?.forEach((p, i) => { pullValues.set([p.x, p.y, p.dx, p.dy], i * 4); radiusValues[i] = p.radius; });
      gl.uniform4fv(pulls, pullValues); gl.uniform4fv(radii, radiusValues);
