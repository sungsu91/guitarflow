...
export function synthesizeString({sampleRate,frequency,string=6,seconds=3,variant=0,muted=false,sustain=false}){
 ...
 if(muted){
  ...
  const decay=sustain?4.5+string*.2:.65+string*.15,loss=Math.exp(-1/(frequency*decay));
 ...
 if(sustain&&!muted){
  // Smooth energy support for connected electric-guitar phrases, without a new pick.
  const window=Math.round(sampleRate*.04);let envelope=1;
  for(let start=0;start<data.length;start+=window){
   const end=Math.min(data.length,start+window);let energy=0;
   for(let i=start;i<end;i++)energy+=data[i]*data[i];
   const target=Math.min(24,.085*Math.exp(-start/(sampleRate*5))/Math.max(.0001,Math.sqrt(energy/(end-start))));
   for(let i=start;i<end;i++){envelope+=(target-envelope)/Math.max(1,sampleRate*.03);data[i]*=envelope;}
  }
 }
...
export function getStringBuffer(audio,position,duration,{variant=0,muted=false,sustain=false}={}){
 ...
 const key=[position.stringNumber,position.midi,seconds,variant,muted,sustain].join(':');
 ...
 const samples=synthesizeString({sampleRate:audio.sampleRate,frequency:position.frequency,string:position.stringNumber,seconds,variant,muted,sustain});
