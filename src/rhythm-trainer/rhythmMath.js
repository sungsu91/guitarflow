...
export function validateBeat(beat) {
  if(!Array.isArray(beat)||!beat.length||beat.length>8)return false;
  ...
  if(beat.reduce((sum,n)=>sum+units(n),0)!==420)return false;
  if(beat.some(n=>n.ticks===4&&!n.tuplet))return beat.length===3&&beat.every(n=>n.ticks===4&&!n.tuplet&&!n.written);
  if(!beat.every(n=>n.tuplet ? [3,5,6,7].includes(n.tuplet.count)&&n.tuplet.normal===(n.tuplet.count===3?2:4)&&Number.isInteger(n.tuplet.group)&&[3,6].includes(n.written)&&Math.abs(n.ticks-n.written*n.tuplet.normal/n.tuplet.count)<1e-8 : [3,6,9,12].includes(n.ticks)&&n.written===undefined))return false;
