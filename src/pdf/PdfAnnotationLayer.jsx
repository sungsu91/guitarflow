...
export default function PdfAnnotationLayer({paperRef,size,crop,page,pageEdit,tool,editing,pen,onStroke,onUpdateStroke,onDeleteStroke,noteDraft,onNoteDraft,onSaveNote,onUpdateNote,onDeleteNote,onCancelNote,onTextPoint,onSelectNote,cropDraft,onCropDraft}){
 ...
 return <div className="pdfAnnotationLayer" data-tool={editing?tool:'view'}>
  ...
  {draft&&<form ref={form} className="pdfInlineText" aria-label="PDF 텍스트 메모 편집" style={{left:Math.max(0,Math.min(notePos.x*size.width,size.width-190)),top:`${notePos.y*100}%`,maxWidth:Math.min(240,size.width)}} onPointerDown={e=>e.stopPropagation()} onClick={e=>e.stopPropagation()} onSubmit={e=>{e.preventDefault();onSaveNote();setProperties(false);}}>
   <input ref={text} aria-label="악보 위 메모" enterKeyHint="done" maxLength={1000} style={{width:`${Math.max(70,Math.min(210,(draft.text.length+2)*size.width/crop.width*draft.size))}px`,fontSize:Math.max(16,size.width/crop.width*draft.size),color:ANNOTATION_COLORS[draft.color]}} value={draft.text} onChange={e=>onNoteDraft({...draft,text:e.target.value})} onKeyDown={e=>{if(e.key==='Escape'){e.preventDefault();onCancelNote();}}}/>
   <div className="pdfInlineActions">{existingNote&&<button aria-label="메모 이동" type="button" title="끌어서 이동" style={{touchAction:'none'}} onPointerDown={e=>{capture(e,{kind:'note'});if(gesture.current)gesture.current.node=form.current;}} {...gestures}><Move size={16}/></button>}<button type="submit" aria-label="메모 저장" disabled={!draft.text.trim()}><Check size={17}/></button><button type="button" aria-label="메모 취소" onClick={()=>{setProperties(false);onCancelNote();}}><X size={17}/></button>{existingNote&&<><button type="button" aria-label="메모 속성" onClick={()=>setProperties(v=>!v)}><Settings2 size={16}/></button><button type="button" aria-label="메모 삭제" onClick={()=>onDeleteNote(draft.id,page)}><Trash2 size={16}/></button></>}</div>
   {properties&&<div className="pdfInlineProperties"><label>크기<input aria-label="메모 크기" type="range" min=".015" max=".08" step=".005" value={draft.size} onChange={e=>onNoteDraft({...draft,size:Number(e.target.value)})}/></label><div>{Object.entries(ANNOTATION_COLORS).map(([color,value])=><button key={color} type="button" className="pdfColorChoice" aria-label={`메모 색상 ${color}`} aria-pressed={draft.color===color} style={{'--swatch':value}} onClick={()=>onNoteDraft({...draft,color})}/>)}</div></div>}
  </form>}
  {selectedStroke&&editing&&tool==='select'&&<div className="pdfInkSelection" style={{left:`${Math.max(0,Math.min(.65,projection({x:selectedStroke.points[0][0],y:0,width:0,height:0}).x))*100}%`,top:`${Math.max(0,Math.min(.93,projection({x:0,y:selectedStroke.points[0][1],width:0,height:0}).y))*100}%`}}><button type="button" aria-label="선택 필기 삭제" onClick={()=>{onDeleteStroke(selectedStroke.id,page);setSelected(null);}}><Trash2 size={16}/></button><button type="button" aria-label="선택 필기 속성" onClick={()=>setProperties(v=>!v)}><Settings2 size={16}/></button><button type="button" aria-label="필기 선택 해제" onClick={()=>{setSelected(null);setProperties(false);}}><X size={16}/></button>{properties&&<div className="pdfSelectedInkProperties"><div>{Object.entries(ANNOTATION_COLORS).map(([color,value])=><button type="button" key={color} className="pdfColorChoice" aria-label={`선택 필기 색상 ${color}`} style={{'--swatch':value}} onClick={()=>onUpdateStroke({...selectedStroke,color},page)}/>)}</div><label>두께<input type="range" aria-label="선택 필기 두께" min=".001" max=".01" step=".001" value={selectedStroke.width} onChange={e=>onUpdateStroke({...selectedStroke,width:Number(e.target.value)},page)}/></label></div>}</div>}
