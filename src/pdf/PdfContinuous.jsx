...
export default function PdfContinuous({pageCount,pageEdits,onPageSeen,...props}) {
 ...
 useEffect(()=>{if(!pdf)return;let live=true;
  (async()=>{const sizes={};for(const n of [active,...(neighbors?[active-1,active+1]:[])]){if(n<1||n>pdf.numPages||!live)continue;const page=await pdf.getPage(n),v=page.getViewport({scale:1});sizes[n]={ratio:v.height/v.width,width:v.width};}if(live)setRatios(old=>({...old,...sizes}));})().catch(e=>{if(live)setError(formatMessage(ko["pdf.couldnTReadPageSizeValue1"], { value1: e.message }));});return()=>{live=false;};
