...
export default function EditablePractice({document,mobile,onClose,onEdit,editing}){
 ...
 return <section className="scoreLibraryPractice">
  ...
  {result.score&&<><article className="etudeSheet"><Score playPosition={playPosition} etude={result.score} mobile={mobile} bpm={bpm}/></article><label className="scoreLibraryTempo">연습 BPM<input aria-label="내 악보 연습 BPM" type="number" min="30" max="240" value={bpm} onChange={e=>setBpm(Math.max(30,Math.min(240,Number(e.target.value)||60)))}/></label><ScorePlayback onPosition={setPlayPosition} score={result.score} bpm={bpm} disabled={editing||Boolean(result.issues?.length)}/></>}
 </section>;
}
