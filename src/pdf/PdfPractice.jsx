...
export default function PdfPractice({initial,blob,mobile,onClose,onInfo,closeController,initialEditing=false}) {
 ...
 const resetPractice=()=>{metro.stop();resumeEditedBar.current=false;const first=order[0]??bars[0];setActiveBar(first?.number??1);goPage(first?.page??1);};
 ...
 const settings=<>
  ...
  {!mobile&&<label><Translation id="pdf.bpmQuarterNoteBeat" /><input aria-label={translateUi("originalUi.pdfBpm")} type="number" min="30" max="240" value={record.bpm} onChange={e=>changeBpm(e.target.value)}/><span className="pdfBpmSteps"><button type="button" aria-label={translateUi("pdf.decreaseBpmBy10")} onClick={()=>changeBpm(record.bpm-10)}>−10</button><button type="button" aria-label={translateUi("pdf.increaseBpmBy10")} onClick={()=>changeBpm(record.bpm+10)}>+10</button></span></label>}
