...
- Progress speeds: 0.75×, 1×, 1.25×, 1.5×. Fall duration, authored spawn interval and recovery interval are divided by this value. Microphone analysis, projectile timing, combo/scoring, count-in voices and audio BPM are not sped up. Settings are session-local and locked during play/pause.
...
- The difficulty header opens a card-based panel with speed controls. Mobile uses a bottom sheet, desktop a centered dialog, and landscape has its own entry button. Dialog supports Escape, focus return/trapping, scroll locking and touch-sized buttons.
