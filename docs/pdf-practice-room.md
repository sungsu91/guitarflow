...
## 2026-09-15 두 점 클릭으로 마디 지정

- 앞서 기록한 드래그 및 별도 ‘나누어 저장’ 조작을 대체했습니다. 마디 설정에서 왼쪽 위와 오른쪽 아래를 한 번씩 클릭/터치하면 영역 가운데에 1·2·3·4 버튼이 나타납니다. 숫자를 누르는 즉시 등분하여 저장하고 다음 영역을 지정할 수 있습니다. 5~16마디 선택도 유지했습니다.
- 첫 점 표시와 마우스 미리보기를 제공하며 Escape 또는 취소로 미확정 영역을 버립니다. 모바일 스크롤 제스처는 영역 생성으로 처리하지 않습니다. 선택창은 화면 가장자리와 하단 조작부를 피하도록 위치를 보정합니다.
- 기존 같은 줄 높이 맞춤, 정규화 좌표, IndexedDB 저장 큐, 개별 마디 선택·삭제, 메트로놈 진행선을 재사용합니다. 기존 마디 데이터와 PDF 원본은 변경하지 않습니다.
- `scripts/verify-pdf-two-click.mjs`: 데스크톱 및 320px 모바일 터치 에뮬레이션에서 1/2/3/4 즉시 저장, 기존 좌표 보존, 같은 줄 맞춤, 취소, 삭제, 재진입 복원, PDF Canvas 재렌더링 없음, 브라우저 오류 없음 확인. 화면과 결과는 `artifacts/pdf-two-click`에 저장합니다. 기존 row-mapping 검증 진입점도 이 검사로 연결했습니다.
- `scripts/verify-pdf-bar-controls.mjs`: 데스크톱/모바일의 기존 마디 선택·삭제, 카운트인 및 연속 진행선, PDF 재렌더링 없는 재생 검사를 통과했습니다. PDF 단위 검사 7개와 프로덕션 빌드도 통과했습니다. 물리적 휴대폰 테스트는 수행하지 않았습니다.


## 2026-09-15 마디 설정 복사와 클릭 표시

- 첫 클릭은 점만 표시합니다. 마우스 이동에 따라 늘어나던 사각형 미리보기를 제거했습니다. 두 번째 클릭 후 마디 수를 선택하는 저장 흐름은 유지합니다.
- 설정된 마디 선택 시 복사/삭제 버튼을 표시합니다. 복사 후 원하는 위치의 왼쪽 위를 한 번 클릭하면 영역의 너비·높이·박 수를 유지한 새 마디를 추가합니다. PDF 내용 복사가 아니라 사용자 지정 영역 복사입니다. 다른 페이지에도 붙일 수 있으며, 용지 끝에서는 영역 전체가 용지 안에 남도록 위치만 보정합니다.
- 복사 취소/Escape와 재생 시작·설정 종료 시 취소를 지원합니다. 기존 마디와 연습 순서를 덮어쓰지 않으며 새 번호를 부여하고 기존 저장 큐를 사용합니다.
- 데스크톱/320px 모바일 에뮬레이션에서 두 점 클릭, 다른 페이지로 설정 복사, 원본 좌표·박 수 보존, 취소, 삭제, 저장 후 복원을 통과했습니다. 기존 진행선·카운트인·Canvas 렌더 횟수 회귀 검사와 빌드도 통과했습니다. 선택 버튼은 모바일에서 각각 44px이며 화면 가장자리 안으로 배치합니다.


## 2026-09-15 PDF 확대 시 세로 스크롤 복구

- 모바일의 높이가 정해지지 않은 flex 부모 안에서 PDF viewport가 원본 용지 높이까지 확장되어 내부 세로 스크롤 범위가 0이 되는 현상을 재현했습니다. 동시에 overscroll containment가 바깥 문서 스크롤을 막았습니다.
- 모바일 viewport에 flex: 0 0 auto를 적용해 기존 화면 높이 기준(세로 62dvh, 가로 70dvh)을 유지합니다. overscroll-behavior:auto로 내부 끝에서 바깥 화면으로 스크롤이 이어집니다. 데스크톱 높이 및 PDF 원본/마디 좌표/저장은 변경하지 않았습니다.
- 100% 선택에 ‘원본 크기’를 표시해 ‘너비 맞춤’과 구분합니다. src/pdf/pdfStudio.css, src/pdf/PdfPractice.jsx 및 scripts/verify-pdf-scroll.mjs가 이번 수정 대상입니다.
- 스크롤 검증은 데스크톱 휠과 Chrome 모바일 터치 이벤트로 fit/100/150%, 마디 설정 ON/OFF를 검사합니다. 스크롤 중 마디/첫 점/분할 선택창이 생성되지 않는지도 확인합니다. 물리적 기기 검증은 아닙니다.
