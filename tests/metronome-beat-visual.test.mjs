...
const finalBeatPass = polishCss.slice(polishCss.lastIndexOf("/* Mobile standalone metronome: keep the strong-beat ready ring quiet"));

test("mobile metronome keeps strong, weak, and mute beat states visually distinct", () => {
  assert.match(finalBeatPass, /--metro-beat-strong-ready: color-mix\(in srgb, var\(--metro-beat-strong\) 22%, var\(--metro-raised-cream\)\)/);
  assert.match(finalBeatPass, /--metro-beat-strong-ready-border: color-mix\(in srgb, var\(--metro-beat-strong\) 58%, var\(--metro-raised-cream\)\)/);
  assert.match(finalBeatPass, /theme-brand[\s\S]*--metro-beat-strong-ready: color-mix\(in srgb, var\(--metro-beat-strong\) 26%, var\(--metro-raised-cream\)\)/);
  assert.match(finalBeatPass, /theme-brand[\s\S]*--metro-beat-strong-ready-border: color-mix\(in srgb, var\(--metro-beat-strong\) 56%, var\(--metro-raised-cream\)\)/);
  assert.match(finalBeatPass, /beatDot--strong[\s\S]*\.beatDot__glyph \{[\s\S]*border: 2px solid var\(--metro-beat-strong-ready-border\) !important/);
  assert.match(finalBeatPass, /beatDot--strong[\s\S]*\.beatDot__innerMark \{[\s\S]*position: absolute !important;[\s\S]*width: calc\(100% \+ 10px\) !important;[\s\S]*border: 2px solid var\(--metro-beat-strong-ready-border\) !important;[\s\S]*transform: translate\(-50%, -50%\) !important/);
...
test("current beat adds an outer glow and restores the full rose-pearl strong-beat color", () => {
  ...
  assert.match(finalBeatPass, /beatDot--strong\.metronomeBeatButton\.active[\s\S]*\.beatDot__glyph \{[\s\S]*border-color: var\(--metro-beat-strong\) !important/);
  assert.match(finalBeatPass, /beatDot--strong\.metronomeBeatButton\.active[\s\S]*\.beatDot__innerMark \{[\s\S]*border-color: var\(--metro-beat-strong\) !important/);
});
