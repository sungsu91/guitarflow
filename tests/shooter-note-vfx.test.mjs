...
test('pitch judgment, projectile scoring and collision geometry remain identical after authorized pacing changes',()=>{
 ...
 // Sprite-pet presentation shipped in ff6d335a; gameplay guards above still apply.
 releasedPaths.push('src/shooter/ShooterPetControls.jsx','src/shooter/ShooterSpritePet.jsx','src/shooter/petAnimation.js','src/shooter/petPreferences.js','src/shooter/skin-picker-frame.css','src/shooter/sprite-pets.css','src/shooter/usePetPlacement.js','src/shooter/usePetPreferences.js');
 releasedPaths.push(...execFileSync('git',['ls-tree','-r','--name-only','ff6d335a','--','public/assets/pets/fretiva_pet_sprite_pack_v1'],{encoding:'utf8'}).trim().split('\n').filter(Boolean));
