...
test("PWA manifest exposes JUST PLAY with separate regular and maskable icons", () => {
  ...
  assert.deepEqual(
    ...
    [
      { src: "/icons/just-play-icon-192.png", sizes: "192x192", type: "image/png", purpose: "any" },
      { src: "/icons/just-play-icon-512.png", sizes: "512x512", type: "image/png", purpose: "any" },
      { src: "/icons/just-play-icon-maskable-192.png", sizes: "192x192", type: "image/png", purpose: "maskable" },
      { src: "/icons/just-play-icon-maskable-512.png", sizes: "512x512", type: "image/png", purpose: "maskable" },
    ],
  );
});
...
test("document metadata connects JUST PLAY names and platform icons", () => {
  ...
  assert.match(html, /<link rel="apple-touch-icon" sizes="180x180" href="\/icons\/just-play-apple-touch-icon\.png"/);
  assert.match(html, /<link rel="shortcut icon" href="\/just-play-favicon\.ico"/);
...
test("Vercel revalidates manifest and JUST PLAY icon metadata", () => {
  ...
  assert.equal(cacheControlBySource.get("/icons/just-play-(.*)"), revalidate);
