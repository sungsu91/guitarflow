...
test("shooter uses the mobile-master HUD at mobile and desktop sizes", async () => {
  ...
  assert.match(appSource, /aria-pressed=\{shooterSolfegeOn\}/);
  assert.match(appSource, /<span>LANG<\/span>/);
  assert.match(appSource, /<span className=\{shooterSolfegeOn \? "active" : ""\}>KO<\/span>/);
  assert.match(appSource, /<span className=\{!shooterSolfegeOn \? "active" : ""\}>EN<\/span>/);
  ...
  assert.match(appCss, /\.mobileShooterPrimaryHudRow \{[\s\S]*grid-template-columns: 82px minmax\(0, 1fr\) 58px 48px/);
  assert.match(appCss, /\.mobileShooterPrimaryHudRow \{[\s\S]*background: rgba\(8, 14, 13, 0\.82\)/);
  assert.match(appCss, /button\.mobileShooterSolfegeHud[\s\S]*height: 40px/);
  assert.match(appCss, /button\.mobileShooterSolfegeHud > b \{[\s\S]*width: 36px;[\s\S]*border-top: 1px solid rgba\(239, 193, 105, 0\.34\)/);
  assert.match(appCss, /button\.mobileShooterSolfegeHud > b > span\.active \{[\s\S]*color: #ffe09a !important;[\s\S]*text-shadow:/);
  assert.match(appCss, /\.mobileShooterPlayHelpMessageBar \{[\s\S]*width: 100%/);
  assert.match(appCss, /\.mobileShooterPlayHelpTooltip \{[\s\S]*top: 60px;[\s\S]*left: 50%;[\s\S]*width: min\(240px, calc\(100% - 24px\)\)/);
  assert.match(appCss, /\.shooterArena \.mobileShooterPlayHelpTooltip \{\s*top: 87px;/);
