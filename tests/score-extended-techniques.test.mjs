...
test('full bend phases use written time and keep exact endpoints',()=>{
 ...
 scheduleScoreExpressions({detune:{setValueCurveAtTime:(curve,at,len)=>calls.push({curve,at,len}),setValueAtTime:()=>{}}},phrase,10);assert.equal(calls[0].at,10);assert.ok(Math.abs(calls[0].len-1)<1e-6);assert.equal(calls[0].curve[0],0);assert.equal(calls[0].curve.at(-1),0);assert(Math.max(...calls[0].curve)>195);
 const resumed=voicesFrom({voices:[phrase]},.75)[0];calls.length=0;scheduleScoreExpressions({detune:{setValueCurveAtTime:(curve,at,len)=>calls.push({curve,at,len}),setValueAtTime:()=>{}}},resumed,20);assert.equal(calls[0].curve[0],100);assert.equal(calls[0].at,20);assert.ok(Math.abs(calls[0].len-.25)<1e-6);
});
